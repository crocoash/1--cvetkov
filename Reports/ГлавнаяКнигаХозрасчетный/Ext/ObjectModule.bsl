...
Функция ЗаголовокОтчета() Экспорт
	Возврат НСтр("ru='Главная книга';uk='Головна книга'");
...
Процедура ВывестиШапкуТаблицы(СтруктураПараметров, Выборка, ШиринаВыводимогоРаздела)
	...
	НачалоЗаголовка.Параметры.ОписаниеСчета = НСтр("ru='Главная книга. Счет ';uk='Головна книга. Рахунок '")+Выборка.СчетПредставление+" """+Выборка.СчетНаименование+"""";
...
Процедура ВывестиСчет(ОбщийИтог, СтруктураПараметров, ПоказыватьЗаголовок)
	...
	Состояние(НСтр("ru='Сбор оборотов по счету ';uk='Збір оборотів по рахунку '")+ОбщийИтог.СчетПредставление);
...
Функция ПолучитьСписокРасшифровок(ПараметрыРасшифровки, ОтчетПоПроводкам = Истина)
	...
	Если ПараметрыРасшифровки<>Неопределено Тогда
		...
		СписокРасшифровки.Добавить(КопияРасшифровки, НСтр("ru='Обороты счета ';uk='Обороти рахунку '") + СтрокаСчета);
		
		Если ОтчетПоПроводкам Тогда
			КопияРасшифровки = БухгалтерскиеОтчеты.СоздатьКопиюСоответствияСтруктуры(ПараметрыРасшифровки);
			КопияРасшифровки.Вставить("ИмяОбъекта","ОтчетПоПроводкамХозрасчетный");
			СписокРасшифровки.Добавить(КопияРасшифровки, НСтр("ru='Отчет по проводкам ';uk='Звіт по проводках '") + СтрокаСчета);
		КонецЕсли;
