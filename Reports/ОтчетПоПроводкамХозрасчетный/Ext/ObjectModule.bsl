...
Функция ПолучитьОписаниеНастроек()
	...
	Если ПоПодстрокеСодержание Тогда
		СтрокаОписания = СтрокаОписания + ?(ПустаяСтрока(СтрокаОписания),"","; ")+НСтр("ru='Содержание содержит ''")+Строка(Подстрока)+"'";
	КонецЕсли;
	Если ПоПодстрокеСубконто Тогда
		СтрокаОписания = СтрокаОписания + ?(ПустаяСтрока(СтрокаОписания),"","; ")+НСтр("ru='Субконто содержит ''")+Строка(Подстрока)+"'";
	КонецЕсли;
