...
Перем мПараметрыРаботыЗвит1С;                  // Кеш параметров 1С-Звит

Перем мМинимальнаяВерсия1СЗвит;                // Минимальная версия 1С-Звит обеспечивающая корректный вызов методов компоненты
...
Функция ДополнитьКешТаблицу(ТекущийОтчет, КешТаблица)
	...
	Если ТекОтчетМетаданные = Неопределено Тогда
		Если ФлагОтладки ИЛИ ФлагОтладкиПакетноеТестирование Тогда
			Сообщить("1С-Звит. Заполнение соответствия отчетов <-> поддерживаемых схем Звит. Ошибка получения метаданных отчета """+ТекущийОтчет+"""!", СтатусСообщения.Внимание);
		КонецЕсли;
	...
	Если МакетПоказателиЗвит1С.Области.Найти("Структура") = Неопределено Тогда
		Если ФлагОтладки ИЛИ ФлагОтладкиПакетноеТестирование Тогда
			Сообщить("1С-Звит. Заполнение соответствия отчетов <-> поддерживаемых схем Звит. В отчете """+ТекущийОтчет+""" в макете ПоказателиЗвит1С отсутствует служебная область ""Структура""!", СтатусСообщения.Внимание);
		КонецЕсли;
		Возврат ДобавленныеСтроки;
	КонецЕсли;
	...
	Для сч = 1 По ОбластьСтруктура.ВысотаТаблицы Цикл
		...
		Если ФлагОтладки ИЛИ ФлагОтладкиПакетноеТестирование Тогда
			...
			Если МакетПоказателиЗвит1С.Области.Найти(ИмяОбласти) = Неопределено Тогда
				Сообщить("1С-Звит. Заполнение соответствия отчетов <-> поддерживаемых схем Звит. В макете ""ПоказателиЗвит1С"" отчета """+ТекущийОтчет+""" отсутствует область """+ИмяОбласти+""", которая описывает заполнение схемы " + ИмяСхемыЗвит1С+ "!", СтатусСообщения.Внимание);
			КонецЕсли;
			
			Если ТекОтчетМетаданные.Формы.Найти(ИмяФормы) = Неопределено Тогда
				Сообщить("1С-Звит. Заполнение соответствия отчетов <-> поддерживаемых схем Звит. В отчете """+ТекущийОтчет+""" ссылка на несуществующую форму отчета  """+ИмяФормы+""", которая заполненяет схему " + ИмяСхемыЗвит1С + "!", СтатусСообщения.Внимание);
			КонецЕсли;
...
Процедура ЗаполнитьСоответствиеОтчетов(ИсточникОтчета = Неопределено) Экспорт
	...
	Если ХранитьКешВСправочникеРегОтчетов Тогда		
		...
		Состояние(НСтр("ru = 'Проверка актуальности кеша поддерживаемых схем для 1С-Звит'; uk = 'Перевірка актуальності кеша підтримуваних схем для 1С-Звіт'"));
		...
		Попытка
			...
			|	И РегламентированныеОтчеты.КешСхемЗвит1САктуален = ЛОЖЬ
			...
			Состояние(НСтр("ru = 'Кеширование поддерживаемых схем для 1С-Звит'; uk = 'Кешування підтримуваних схем для 1С-Звіт'"));
	
			Если ФлагОтладки Тогда
				Если Не ЗначениеЗаполнено(ИсточникОтчета) Тогда		
					Сообщить("1С-Звит. Обновление доступных схем для всех отчетов (кеш в справочнике рег.отчетов)");
				Иначе		
					Сообщить("1С-Звит. Обновление доступных схем для отчета """+ИсточникОтчета+""" (кеш в справочнике рег.отчетов)");
				КонецЕсли;	
			КонецЕсли;			
			...
			Пока Выборка.Следующий() Цикл
				...
				Если ТекущийОтчет = Неопределено Тогда
					Если ФлагОтладки ИЛИ ФлагОтладкиПакетноеТестирование Тогда
						Сообщить("1С-Звит. Заполнение соответствия. Не удалось получить отчет """+ТекущийИсточникОтчета+"""!", СтатусСообщения.Внимание);
					КонецЕсли;
					Продолжить;
				КонецЕсли;
		...
		Исключение
			...
			Сообщить(НСтр("ru = 'Ошибка обновления кеша поддерживаемых схем ""1С-Звит""'; uk = 'Помилка оновлення кеша підтримуваних схем ""1С-Звіт""'"), СтатусСообщения.Важное);
	...
	Иначе
		...
		// Требутся перезаполнить определенный отчет, но 1С-Звит не использовался. Отложим до первого запроса от 1С-Звит
		...
		Если ФлагОтладки Тогда
			Если Не ЗначениеЗаполнено(ИсточникОтчета) Тогда		
				Сообщить("1С-Звит. Обновление доступных схем для всех отчетов (кеш в памяти)");
			Иначе		
				Сообщить("1С-Звит. Обновление доступных схем для отчета """+ИсточникОтчета+""" (кеш в памяти)");
			КонецЕсли;	
		КонецЕсли;			
...
// Функция проверяет версию 1С-Звит на актуальность
//
Функция ВерсияЗвит1САктуальна(ВыводитьПредупреждение = Истина)
	...
	Попытка
		...
		Если ФлагОтладки ИЛИ ФлагОтладкиПакетноеТестирование Тогда
			Если СтрДлина(ВерсияЗвит1С)<> СтрДлина(мМинимальнаяВерсия1СЗвит) Тогда
				Сообщить("1С-Звит. Метод getZvitVersion() возвратил некорректный номер версии: """+ВерсияЗвит1С+"""", СтатусСообщения.Внимание);
			КонецЕсли;
		КонецЕсли;
	Исключение 
		// Старые версии 1С-Звит не поддерживают данный метод
	...
	Если (ВерсияЗвит1С = Неопределено) ИЛИ (ВерсияЗвит1С < мМинимальнаяВерсия1СЗвит) Тогда
		Если ВыводитьПредупреждение Тогда
			...
			ТекстПредупреждения = Локализация.СтрШаблон1(НСтр("ru = 'Внимание, используется устаревшая версия 1С-Звит!
	                                                          |
	                                                          |Необходмо обновить программу до версии не ниже ¤1¤. Для этого необходмо в окне 1С-Звит
...
// Функция заполняет структуру параметров необходимую для работы 1С-Звит
//
Функция ЗаполнитьПараметрыРаботыЗвит1С()
	...
	// Ошибки генерируемые в 1С-Звит при проверке статусов налоговых документов (исходящих)
	...
	ПредставлениеСостоянийДляИсходящихНалоговыхДокументов.Вставить(-2, НСтр("ru = 'В ""1С-Звит"" обнаружено более одного документа соответствующего документу ""1С""'; uk = 'В ""1С-Звіт"" виявлено більше одного документа що відповідає документу ""1С""'"));
	ПредставлениеСостоянийДляИсходящихНалоговыхДокументов.Вставить(-1, НСтр("ru = 'Не выгружен в ""1С-Звит""'; uk = 'Не вивантажен в ""1С-Звіт""'"));
	ПредставлениеСостоянийДляИсходящихНалоговыхДокументов.Вставить( 0, НСтр("ru = 'Выгружен в ""1С-Звит""'; uk = 'Вивантажен в ""1С-Звіт""'"));
	...
	// Ошибки генерируемые в 1С-Звит при проверке статусов налоговых документов (входящих)
	...
	ПредставлениеСостоянийДляВходящихНалоговыхДокументов.Вставить(-2, НСтр("ru = 'В ""1С-Звит"" обнаружено более одного вытяга соответствующего документу ""1С""'; uk = 'В ""1С-Звіт"" виявлено більше одного витягу що відповідає документу ""1С""'")); // Несуществующее состояние для входящих 
	ПредставлениеСостоянийДляВходящихНалоговыхДокументов.Вставить(-1, НСтр("ru = 'Не сформирован запрос в ""1С-Звит""'; uk = 'Не сформирован запит в ""1С-Звіт""'")); // -1 Еще не реализовано со стороны 1С-Звит, пока используется +2
	ПредставлениеСостоянийДляВходящихНалоговыхДокументов.Вставить( 1, НСтр("ru = 'Запрос сформирован в ""1С-Звит""'; uk = 'Запит сформирован в ""1С-Звіт""'"));       //  0 Еще не реализовано со стороны 1С-Звит, пока используется +2
	...
	мПараметрыРаботыЗвит1С.Вставить("ОшибкаВзаимодейсвияСЗвит1ССтр", НСтр("ru = '<Ошибка взаимодействия с ""1С-Звит"">'; uk = '<Помилка взаємодії з ""1С-Звіт"">'"));
		
КонецФункции 

// Функция возвращает структуру параметров необходимую для работы 1С-Звит
//
...
Функция ЗаполнитьИСохранитьОтчетЗвит1С(ОтчетЗвит1С, СоответствиеОтчета, НастройкаФормыОтчета, ДанныеДляСохраненияРасшифровкиОтчета = Неопределено, ОткрыватьОтчет = Неопределено)
	...
	Если ТекФорма = Неопределено Тогда
		...
		Если ФлагОтладкиПакетноеТестирование Тогда	// При пакетном тестировании сообщения не выводятся, продублируем его
			Сообщить("1С-Звит. "+ТекстОшибки, СтатусСообщения.Важное);
		КонецЕсли;
		Возврат Ложь;
	КонецЕсли;
...
Функция Инициализация(ВыводитьСообщенияОбОшибках) Экспорт
	...
	// Установим представление 1С для 1С-Звит
...
// Функция выполняющая переподключение менеджера 1С-Звит, без обновления компоненты ZviAPI.dll
//
Функция Переинициализация(ВыводитьСообщенияОбОшибках, СтарыйМенеджерЗвит1С) Экспорт
	...
	Исключение
		Если ВыводитьСообщенияОбОшибках Тогда
			Сообщить(НСтр("ru = 'Ошибка переинициализации менеджера ""1С-Звит""!'; uk = 'Помилка переініціалізаціі менеджера ""1С-Звіт""!'"), СтатусСообщения.Важное);
			Сообщить(ОписаниеОшибки(), СтатусСообщения.БезСтатуса);
		КонецЕсли;
		Возврат Ложь;
	КонецПопытки;
	...
	Если ФлагОтладки Тогда
		Сообщить("1С-Звит. Произведена переинициализация менеджера 1С-Звит");
	КонецЕсли;
...
Функция ЗапуситьЗвит1С(НаПереднийПлан = Неопределено, Организация = Неопределено) Экспорт
	...
	Если Компонента.IsZvitActive() = 1 Тогда
		Если Не ВерсияЗвит1САктуальна() Тогда
			// Сессия 1С-Звит запущена для обновления
			Возврат Ложь;
		КонецЕсли;
	...
	Состояние(НСтр("ru = 'Запуск 1С-Звит'; uk = 'Запуск 1С-Звіт'"));	
	...
	// Установлено несколько копий 1С-Звит. При выборе 
	// рабочего каталога с 1С-Звит пользователь нажал "Відмінити"
	Если РезультатЗапуска = Неопределено Тогда
		Предупреждение(НСтр("ru = 'Запуск ""1С-Звит"" отменен. Возможные причины:
                             |
                             | 1. Нажата кнопка ""Відмінити"" в форме выбора рабочего каталога ""1С-Звит""
                             | 2. В каталоге с ""1С-Звит"" остался файл ""zvit1c.ini"" (для запуска ""1С-Звит"" достаточно удалить данный файл)'; uk = 'Запуск ""1С-Звіт"" скасовано. Можливі причини:
                             |
	...
	ИначеЕсли РезультатЗапуска = 0 Тогда
		Предупреждение(НСтр("ru = 'Ошибка подключения к системе ""1С-Звит"". Возможные причины:
                             |
                             | 1. Нажата кнопка ""Відмінити"" в форме выбора рабочего каталога ""1С-Звит""
                             | 2. В каталоге с ""1С-Звит"" остался файл ""zvit1c.ini"" (для запуска ""1С-Звит"" достаточно удалить данный файл)'; uk = 'Помилка підключення до системи ""1С-Звіт"". Можливі причини:
                             |
                             | 1. Натиснута кнопка ""Відмініті"" у формі вибору робочого каталогу ""1С-Звіт""
                             | 2. У каталозі з ""1С-Звіт"" залишився файл ""zvit1c.ini"" (для запуску ""1С-Звіт"" достатньо видалити даний файл)'"));	
		Возврат Ложь;
...
Процедура ОбработкаВнешнегоСобытияЗвит1С(Источник, Событие, Данные) Экспорт
	...
	ИначеЕсли Команда = "REPORT" Тогда
		...
		ИначеЕсли СоответствиеОтчета.ИмяОтчета = "РегламентированныйОтчетДекларацияОПрибыли" Тогда
			// Отчеты за II-III и II-IV кварталы 2011 года в 1С-Звит хранятся
		...
		Если ОрганизацииЗвит1С.Количество() = 0 Тогда
			ТекстОшибки = "В 1С не знайдено фірму з ЄДРПОУ """+ОтчетЗвит1С.FIRM_EDRPOU+""""+?(РегламентированнаяОтчетность.ИДКонфигурации()<> "УТ", " на "+Формат(ДатаНачалаПериодаОтчета, "Л=uk_UA; ДФ=dd.MM.yyyy"), "")+"
		...
		ИначеЕсли ОрганизацииЗвит1С.Количество() > 1 Тогда
			ТекстОшибки = "В 1С знайдено декілька фірм з однаковим ЄДРПОУ """+ОтчетЗвит1С.FIRM_EDRPOU+""""+?(РегламентированнаяОтчетность.ИДКонфигурации()<> "УТ", " на "+Формат(ДатаНачалаПериодаОтчета, "Л=uk_UA; ДФ=dd.MM.yyyy"), "")+Символы.ПС;
...
// Функция открывает форму настройки 1С-Звит
//
...
// Функция открывает форму для пакетной выгрузки налоговых документов в 1С-Звит
//
...
// Функция добавляет переданный налоговый документ в форму пакетной выгрузки и проверки 1С-Звит
//
...
// Функция возвращяает уникальный идентификатор для 1С-Звит
//
...
Функция ПроверитьКлючевыеПараметрыПередПовторнойВыгрузкой(ОтчетЗвит1С, ТекстОшибки, ИмяСхемыЗвит1С, ТипПериода, НомерВПериоде, Год, КодПоЕДРПОУ);
	...
	Иначе
		...
		Если ФлагОтладки ИЛИ ФлагОтладкиПакетноеТестирование Тогда
			Сообщить("1С-Звит. В функцию ПроверитьКлючевыеПараметрыПередПовторнойВыгрузкой() передан некорректный тип периода: "+ТипПериода);
		КонецЕсли;
		Возврат Ложь;
	КонецЕсли;
	...
	Если Не ПустаяСтрока(ТекстОшибки) Тогда
		ТекстОшибки = НСтр("ru = 'Обнаружен отчет ""1С-Звит"" соответсвующий выгружаемому из ""1С"" документу.
...
// Функция выгружает переданный налоговый документ в 1С-Звит
//
Функция ВыгрузитьДокументВЗвит1С(ДокументСсылка, ПараметрыПакетнойВыгрузки = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(Организация.ДатаНачалаИспользованияЗвит1С) Тогда
		ТекстОшибки = НСтр("ru = '""1С-Звит"" не используется для организации ""'; uk = '""1С-Звіт"" не використовується для організації ""'")+Строка(Организация)+"""."+Символы.ПС+
		              НСтр("ru = 'В карточке организации не указана дата начала использования ""1С-Звит""'; uk = 'В картці організації не вказана дата початку використання ""1С-Звіт""'");
		Возврат СформироватьРезультатВыгрузкиДокумента(ТекстОшибки, Истина);
	КонецЕсли;
	...
	ПоследняяАктуальная = ВосстановитьЗначение("Звит1С_НалоговыеДокументы_ВыгружатьПоПоследнейАктуальнойСхеме");
	Если ТипЗнч(ПоследняяАктуальная) <> Тип("Булево") Тогда
		ПоследняяАктуальная = Ложь;
	КонецЕсли;
	...
	Если тзСхемы = Неопределено Тогда
		ТекстОшибки = НСтр("ru = 'Отсутствует поддержка устаревшей схемы для выгрузки текущего документа в ""1С-Звит""'; uk = 'Відсутня підтримка застарілої схеми для вивантаження поточного документа в ""1С-Звіт""'");
		Возврат СформироватьРезультатВыгрузкиДокумента(ТекстОшибки, Истина);
	КонецЕсли;
	...
	Если Не ЗапуситьЗвит1С(Ложь, Организация) Тогда
		ТекстОшибки = НСтр("ru = 'Не удалось запустить ""1С-Звит""'; uk = 'Не вдалося запустити ""1С-Звіт""'");
		Возврат СформироватьРезультатВыгрузкиДокумента(ТекстОшибки, Истина, Истина);
	КонецЕсли;			
	...
	ИначеЕсли РезультатаСоздания = 4 Тогда
		...
		ОтчетЗвит1С = Результат;
		Если ФлагОтладки Тогда
			Сообщить("1С-Звит. Открыт существующий отчет (отклоненный)");
		КонецЕсли;
	...
	ИначеЕсли РезультатаСоздания = -1 Тогда
		ТекстОшибки = НСтр("ru = 'Общая ошибка ""1С-Звит""'; uk = 'Загальна помилка ""1С-Звіт""'");
		Возврат СформироватьРезультатВыгрузкиДокумента(ТекстОшибки, Истина);
	...
	ИначеЕсли РезультатаСоздания = -3 Тогда
		ТекстОшибки = НСтр("ru = 'ЕДРПОУ организации указанной в документе не совпадает с текущем ЕДРПОУ в ""1С-Звит""
	...
	ИмяСхемыЗвит1С = ""; // Очищаем реквизит менеджера 1С-Звит т.к. работа с документом завершена 
	
	Если Не ФлагУспешногоСохранения Тогда
		ТекстОшибки = НСтр("ru = 'Ошибка передачи данных в 1С-Звит'; uk = 'Помилка передачі даних в 1С-Звіт'");
		Возврат СформироватьРезультатВыгрузкиДокумента(ТекстОшибки, Истина);
	КонецЕсли;
...
// Функция проверяет список переданных документов на возможность выгрузки в объект NNStatusQuery/NNRequest  1С-Звит
...
Функция СформироватьТаблицыДляЗапросаСостоянияРегистрации(МассивДокументов, ДляПолученияСостоянияРегистрации)
	...
	Иначе
		...
		// Прочие фатальные ошибки, за исключением ошибки подключения к 1С-Звит
	...
	ТаблицаРезультат.Колонки.Добавить("ДокументКорректен");             // Истина - запрос состояния отправлен в 1С-Звит, иначе Ложь
	...
	Для каждого Документ Из МассивДокументов Цикл
		...
		Если Не ЗначениеЗаполнено(Организация.ДатаНачалаИспользованияЗвит1С) Тогда
			СтрокаРезультат.СостояниеСтр = НСтр("ru = '""1С-Звит"" не используется для организации ""'; uk = '""1С-Звіт"" не використовується для організації ""'")+Строка(Организация)+"""."+Символы.ПС+
			                               НСтр("ru = 'В карточке организации не указана дата начала использования ""1С-Звит""'; uk = 'В картці організації не вказана дата початку використання ""1С-Звіт""'");
			Продолжить;
		КонецЕсли;
		...
		Если РеквизитыКонтрагента = Неопределено Тогда
			РеквизитыКонтрагента  = Новый Структура("ИНН", СокрЛП(Контрагент.ИНН));
			РеквизитыКонтрагентов[Контрагент] = РеквизитыКонтрагента;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(РеквизитыКонтрагента.ИНН) Тогда
			СтрокаРезультат.СостояниеСтр = НСтр("ru = 'В карточке контранета не заполнен ИНН'; uk = 'У картці контранета не заповнений ІПН'");
			Продолжить;
		КонецЕсли;
		...
		// 5. Попробуем запустить 1С-Звит или выведем причины отказа (не установлен, не используется в этой ИБ)
		// один раз в цикле
		Если ЕстьПодключениеКЗвит1С = Неопределено Тогда
			...
			Если ЕстьПодключениеКЗвит1С Тогда				
				...
				Состояние(НСтр("ru = 'Формирование запроса состояния регистрации в ЕРНН в ""1С-Звит""'; uk = 'Формування запиту стану реєстрації в ЄРПН в ""1С-Звіт""'"));			
				Если ДляПолученияСостоянияРегистрации Тогда
					NNTable = Компонента.CreateNNStatusQuery();	
				Иначе
					Попытка
						// Метод CreateNNRequestEx добавлен в 1.00.034 релизе 1С-Звит
						...
						ИначеЕсли РезультатаСоздания = -1 Тогда
							...
							РезультатФомирования.ТекстОшибки = РезультатФомирования.ТекстОшибки + 
							                                   НСтр("ru = 'Общая ошибка ""1С-Звит"" при вызове CreateNNRequestEx("""+РеквизитыОрганизации.КодПоЕДРПОУ+""")'; uk = 'Загальна помилка ""1С-Звіт"" при виклику CreateNNRequestEx("""+РеквизитыОрганизации.КодПоЕДРПОУ+""")'");
							
						ИначеЕсли РезультатаСоздания = -3 Тогда
							РезультатФомирования.ОшибкаФормированияТаблицы = Истина;
							РезультатФомирования.ТекстОшибки = РезультатФомирования.ТекстОшибки + 
							                                   НСтр("ru = 'ЕДРПОУ организации указанной в документе не совпадает с текущем ЕДРПОУ в ""1С-Звит""
					                                                      |Переключите текущую организацию в ""1С-Звит"" и повторите выгрузку еще раз'; uk = 'ЄДРПОУ організації зазначеної в документі не збігається з поточному ЄДРПОУ в ""1С-Звіт""
					                                                      |Переключіть поточну організацію в ""1С-Звіт"" і повторіть вивантаження ще раз'");
						КонецЕсли;
		...
		Если СтрокаРезультат.ЭтоИсходящийДокумент Тогда
			...
			// Для проверки ключевых полей в 1С и 1С-Звит
			...
			Если ЗначениеЗаполнено(ТекущийСпецРежимНалогообложения) Тогда
				// Или значение или пусто - пробел не выгружаем 1С-Звит пробел не выгружается, не сохранятся, вручную не добавляется
			...
			Если мПараметрыРаботыЗвит1С.УчетПоОбособленнымПодразделениям Тогда
				...
				Если ЗначениеЗаполнено(ТекущееОбособленноеПодразделение) Тогда
					...
					НомерФилиала  = ?(ЗначениеЗаполнено(Префикс), Прав("0000" + СокрЛП(Префикс), 4), "");  //число, сведущими нулями, 4 символа 
		...
		Иначе
			...
			ТекстОшибкиФормата = НСтр("ru = 'Номер входящего документа не соответсвует формату: Н, Н/С или Н/СФ, где Н - порядковый номер документа, С - спец.режим или пробел (один символ), Ф - номер филиала (от одного до четырех символов)'; 
					                  |uk = 'Номер вхідного документа не відповідає формату: Н, Н/С або Н/СФ, де Н - порядковий номер документа, С - спец.режим або пробіл (один символ), Ф - номер філії (від одного до чотирьох символів)'");
...
Функция ПолучитьСостояниеРегистрацииДокументов(МассивДокументов) Экспорт
	...
	ТаблицаРезультат.Колонки.Добавить("Звит1С_СовпадаютКлючевыеПоля");           // только исх. Если документ не выгружен то Неопределено, иначе Булево - ложь в случае когда документ выгружен из 1С и в 1С-Звит различаются ключевые поля
	ТаблицаРезультат.Колонки.Добавить("Звит1С_ДокументЗарегистрирован");         // Булево - признак регистрации документа в ЕРНН 
	ТаблицаРезультат.Колонки.Добавить("Звит1С_СуммаНДС");                        // Сумма НДС по данным 1С-Звит
	ТаблицаРезультат.Колонки.Добавить("Звит1С_ДатаРегистрации");                 // Дата регистрации по данным 1С-Звит
	ТаблицаРезультат.Колонки.Добавить("Звит1С_Состояние");                       // Если ошибка в 1С, то Звит1С_Состояние = Неопределено, иначе число возвращенное из 1С-Звит
	...
	Состояние(НСтр("ru = 'Исполнение запроса состояния регистрации в ЕРНН в ""1С-Звит""'; uk = 'Виконання запиту стану реєстрації в ЄРПН в ""1С-Звіт""'"));
	NNStatusResult = Компонента.ExecNNStatusQuery(NNStatusQuery);
	
	// 3. Обработка запроса
	Состояние(НСтр("ru = 'Обработка состояния регистрации в ЕРНН в ""1С-Звит""'; uk = 'Обробка запиту стану реєстрації в ЄРПН в ""1С-Звіт""'"));
		...
		Если (СтрокаРезультат.ЭтоИсходящийДокумент) 
		   И (СтрокаРезультат.Звит1С_Состояние > -1) Тогда	// Документ выгружен	
			...
			Для Каждого КлючИЗначение Из ДанныеДокументаЗвит1С Цикл // Приводим незаполненные значения к строкам
				...
				КонецЕсли;
			КонецЦикла;
			// В режиме отладки в числовые поля документа 1С-Звит вместо пустых значений выгружаются нули
			...
			КонецЕсли;
			Если СтрокаРезультат._1С_Документ_Номер <> ДанныеДокументаЗвит1С.Номер Тогда
			...
			КонецЕсли;
			Если СтрокаРезультат._1С_Документ_Филиал <> ДанныеДокументаЗвит1С.Филиал Тогда
				ТекстОшибки = ТекстОшибки + НСтр("ru = 'филиал'; uk = 'філія'")+", ";
			КонецЕсли;
			Если НачалоДня(СтрокаРезультат._1С_Документ_Дата) <> НачалоДня(ДанныеДокументаЗвит1С.Дата) Тогда
				ТекстОшибки = ТекстОшибки + НСтр("ru = 'дата'; uk = 'дата'")+", ";
			КонецЕсли;
			Если СтрокаРезультат._1С_СуммаНДС <> СтрокаРезультат.Звит1С_СуммаНДС Тогда
...
Функция СформироватьЗапросСостоянияРегистрацииДокументов(МассивДокументов) Экспорт
	...
	// Прочие ошибки, за исключением ошибки подключения к 1С-звит
	...
	Состояние(НСтр("ru = 'Формирование и отправка запроса состояния регистрации в ЕРНН в ""1С-Звит""'; uk = 'Формування та відправлення запиту стану реєстрації в ЄРПН в ""1С-Звіт""'"));
	...
	Исключение
		...
		КонецЦикла;
		Если ФлагОтладки ИЛИ ФлагОтладкиПакетноеТестирование Тогда
			Сообщить("1С-Звит. Ошибка отправки запроса НН: "+ТекстОшибки);
		КонецЕсли;
...
// Функция открывает налоговый документ в 1С-Звит
// 
