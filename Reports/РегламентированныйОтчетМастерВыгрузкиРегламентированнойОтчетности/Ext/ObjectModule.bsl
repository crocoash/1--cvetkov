...
Функция РазборкаСхемы(ИмяСхемы, тбПоказателиШапка,тбПоказателиТело, фОшибка)
	...
	Попытка
		...
		Если НужныйОтчет = Неопределено Тогда
			Сообщить(Локализация.СтрШаблон1(НСтр("ru='Не найден отчет ¤1¤!. Экспорт xml по данной схеме не будет выполнен !';uk='Не знайдений звіт ¤1¤!. Експорт xml за даною схемою не буде виконаний !'"), ИмяНужногоОтчета));
			фОшибка = 1;
	...
	Если не ФайлСхемы.Существует() Тогда
		...
		Сообщить(Локализация.СтрШаблон1(НСтр("ru='Не найден файл схемы ¤1¤!. Экспорт xml по данной схеме не будет выполнен !';uk='Не знайдений файл схеми ¤1¤!. Експорт xml за даною схемою не буде виконаний !'"), ИмяФайлаСхемы));
		фОшибка = 1;
		Возврат Неопределено;
	КонецЕсли;
...
Функция ПреобразоватьТипXMLОбщий(Значение,Тип,Результат,ОписаниеОшибки = "")
	...
	ИначеЕсли ВРег(Тип) = ВРег("DGUKTZED")
		...
		Если  Длина <> 4
			...
			ОписаниеОшибки = "Значение не является набором цифр (4, 6 или 10-и значным)";
	...
	ИначеЕсли ВРег(Тип) = ВРег("DGPNtypr") Тогда
		...
		ИначеЕсли СтрДлина(Результат) <> 2
			...
			ИЛИ (Лев(Результат, 1) = "1" И Найти("01234", Прав(Результат, 1)) = 0) Тогда
