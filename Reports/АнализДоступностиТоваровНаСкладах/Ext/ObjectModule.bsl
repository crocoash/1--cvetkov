...
Процедура УстановитьНачальныеНастройки(ДополнительныеПараметры = Неопределено) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ГруппыХарактеристик.Группа.ИгнорироватьНоль
	|			ТОГДА ГруппыХарактеристик.Группа.ИНКоличество
	|		ИНАЧЕ """"
	|	КОНЕЦ КАК ИНКоличество,
	|	ВЫБОР
	|		КОГДА ГруппыХарактеристик.Группа.ИгнорироватьНоль
	|			ТОГДА ВЫБОР
	|					КОГДА ГруппыХарактеристик.Группа.ДатаПошиваНачало = ДАТАВРЕМЯ(1, 1, 1)
	|						ТОГДА ГруппыХарактеристик.Группа.Владелец.СрокИсполнения1
	|					ИНАЧЕ ГруппыХарактеристик.Группа.ДатаПошиваНачало
	|				КОНЕЦ
	|		ИНАЧЕ """"
	|	КОНЕЦ КАК ДатаПошиваНачало
	|{ВЫБРАТЬ
	|	Склад.*,
	|	Номенклатура.*,
	...
	|			МАКСИМУМ(ГруппыХарактеристикХарактеристики.Ссылка) КАК Группа
	|		ИЗ
	...
	|		
	|		СГРУППИРОВАТЬ ПО
	|			ГруппыХарактеристикХарактеристики.Характеристика) КАК ГруппыХарактеристик
	|		ПО ВложенныйЗапрос.ХарактеристикаНоменклатуры = ГруппыХарактеристик.Характеристика
	|{ГДЕ
	|	ВложенныйЗапрос.Качество.* КАК Качество,
	|	ВложенныйЗапрос.КоличествоОстаток КАК КоличествоОстаток,
	|	ВложенныйЗапрос.КоличествоВРезерве КАК КоличествоВРезерве,
	...
	УниверсальныйОтчет.мСтруктураПредставлениеПолей.Вставить("ИНКоличество", "ИН количество");  
	УниверсальныйОтчет.мСтруктураПредставлениеПолей.Вставить("ДатаПошиваНачало", "ДатаПошиваНачало");
	...
	УниверсальныйОтчет.ДобавитьПоказатель("ИНКоличество", "ИН количество", Истина, "ЧЦ=10; ЧДЦ=; ЧН=0"); 
	УниверсальныйОтчет.ДобавитьПоказатель("ДатаПошиваНачало","ДатаПошиваНачало", Истина, "ДФ='dd.MM.yyyy'");
