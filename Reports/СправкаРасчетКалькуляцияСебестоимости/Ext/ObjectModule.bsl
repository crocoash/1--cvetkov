...
Функция СформироватьТекстЗапроса(НачалоПериода, Организация)
	...
	Если ИспользуемыеКлассыСчетовРасходов = Перечисления.КлассыСчетовРасходов.Класс8и9 Тогда
		ТекстПодзапросаПрямыеЗатраты = "
			...
			|   ИсходныеПроводки.СчетКт КАК КорСчетПрямыхЗатрат,
			|   ИсходныеПроводки.СубконтоКт1 КАК КорСубконтоПрямыхЗатрат,
			|   СУММА(ИсходныеПроводки.Сумма) КАК СуммаПрямыхЗатрат,
			|   СУММА(ИсходныеПроводки.СуммаНУДт) КАК СуммаПрямыхЗатратНУ,
			|   СУММА(ИсходныеПроводки.КоличествоКт) КАК КоличествоПрямыхЗатрат
			|ИЗ
			...
			|                          И (НЕ СчетДт В ИЕРАРХИИ (&Счет91)) И (НЕ СчетДт В ИЕРАРХИИ (&Счет24))) КАК ХозрасчетныйДвиженияССубконто
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок, ВысотаЗаголовка, ТолькоЗаголовок = Ложь, НачалоПодписи, ПоказыватьПодписи) Экспорт
	...
	ЗаголовокОтчета.Параметры.НазваниеОтчета      = НСтр("ru='Калькуляция себестоимости продукции (бухгалтерский учет)';uk='Калькуляція собівартості продукції (бухгалтерський облік)'");
	...
	Пока ВыборкаПодразделение.Следующий() Цикл		
		...
		Пока ВыборкаНоменклатурнаяГруппа.Следующий() Цикл
			...
			Пока ВыборкаНоменклатураВыпуска.Следующий() Цикл
				...
				ОбластьШапка.Параметры.ТекстКоличество             = ?(ВыборкаНоменклатураВыпуска.Количество = 0, "", НСтр("ru='Количество выпуска:';uk='Кількість випуску:'"));
				ОбластьШапка.Параметры.ТекстСумма                  = ?(ВыборкаНоменклатураВыпуска.Количество = 0, "", НСтр("ru='Факт. стоимость выпуска:';uk='Факт. вартість випуску:'"));
				ОбластьШапка.Параметры.СебестоимостьЕдиницы        = НСтр("ru='Себестоимость ед.';uk='Собівартість од.'");
	...
	ДокументРезультат.ИмяПараметровПечати = НСтр("ru='Калькуляция себестоимости';uk='Калькуляція собівартості'");
...
Функция РазъяснитьЗатраты(СчетЗатрат)
	...
	Если СчетЗатрат.ПринадлежитЭлементу(СчетУчетаРасчетов)
		...
		Рез = НСтр("ru='Услуги сторонних организаций';uk='Послуги сторонніх організацій'");
	Иначе
		Рез = СчетЗатрат.Наименование;
	КонецЕсли;
	
	Рез = Рез + НСтр("ru=' (сч. ';uk=' (рах. '") + СокрЛП(СчетЗатрат.Код) + ")";
	Возврат Рез;
