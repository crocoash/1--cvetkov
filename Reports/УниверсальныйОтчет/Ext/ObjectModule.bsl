...
Процедура СформироватьОтчет(ТабличныйДокумент, Очищать = Истина, ФиксироватьЗаголовок = Истина, Отчет = Неопределено) Экспорт
	...
	Если ВыводВозможен() Тогда
		...
		ПостроительОтчета.Параметры.Вставить("ДатаПервыхЦен", Дата('00010101000000')); 		
...
Процедура УстановитьНачальныеНастройкиПоМетаданнымРегистра(ИмяРегистра)
	...
	ДобавитьСвойстваИКатегорииРегионов = Ложь;
	
	Для каждого Измерение из МетаданныеРегистра.Измерения Цикл
		
		Если Измерение.Имя = "Контрагент" Тогда
			
			ОписаниеПолейВыборки = ОписаниеПолейВыборки + ?(ОписаниеПолейВыборки = "", "", ",") + "
				|	ИсточникДанных.Контрагент.Регион КАК Регион,
				|	ПРЕДСТАВЛЕНИЕ(ИсточникДанных.Контрагент.Регион) КАК РегионПредставление";
			...
			мСтруктураПредставлениеПолей.Вставить("Регион", "Регион");
			
			Если ИспользоватьСвойстваИКатегории Тогда
				
				ДобавитьСвойстваИКатегорииРегионов = Истина;
	...
	Если ДобавитьСвойстваИКатегорииРегионов Тогда
		
		ДобавитьСвойстваИКатегорииДляПоля("ИсточникДанных.Контрагент.Регион", "Регион", "Регион", ПланыВидовХарактеристик.НазначенияСвойствКатегорийОбъектов.Справочник_Регионы);
