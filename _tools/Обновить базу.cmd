...
echo ==========================================
echo.
echo   1  - Обновить базу (сам решит, надо ли кого-то выгонять)
echo   2  - Обновить динамически, никого не трогать
echo   3  - Только загрузить из файлов (можно при людях)
echo   4  - Только обновить конфигурацию БД
echo   5  - То же, никого не трогая
echo   6  - Выгрузить конфигурацию из базы в файлы + снимок
echo   7  - Настроить базу и пользователя
echo   8  - Обновить БД, выгнав всех сразу (без вопроса)
echo.
echo   0  - Выход
echo.
set OPT=
...
echo Правки без изменения структуры применяются на живой базе - никого не трогаем.
echo Если правки структурные - покажет, кто в базе, и спросит: "д" - выгнать и обновить.
echo.
