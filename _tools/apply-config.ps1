...
# монопольному режиму, сколько их ни жди.
#
# Чужие клиентские сеансы сами по себе не завершаются, но и молча в тупик не заводят:
# если в базе работают люди, скрипт спрашивает прямо в консоли, выгонять ли их, —
# и до запуска Конфигуратора, и повторно, если обновление всё-таки упёрлось в
# блокировку. Согласие — «д»: скрипт запрещает начало сеансов, завершает все текущие
# и доделывает обновление тем же запуском (загрузку из файлов не повторяет, она уже прошла).
# Ключ -ВыгнатьВсех = то же самое без вопроса; при запуске без человека за консолью
# (перенаправленный ввод, планировщик) вопрос не задаётся вовсе.
...
function Запустить1С([string[]]$Аргументы, [string]$Что, [string[]]$ПовторАргументы, [string]$ПовторЧто) {
    ...
    if ($p.ExitCode -ne 0) {
        ...
        if ($текст -match "для конфигурирования") {
            ...
            # Обычный сценарий: обновление упёрлось в чужие сеансы, а человек сидит
            # перед экраном и готов их выгнать. Спрашиваем прямо здесь и доделываем
            # работу тем же запуском — не заставляя вспоминать ключи и запускать заново.
            if ($ПовторАргументы -and (МожноВыгонять)) {
                Write-Host ""
                Write-Host "Могу завершить эти сеансы прямо сейчас и повторить обновление." -ForegroundColor Yellow
                Write-Host "У работающих в базе несохранённые данные потеряются." -ForegroundColor Yellow
                $ответ = Read-Host "«д» — выгнать и обновить, Enter — не трогать"
                if ($ответ -match "^\s*(д|d|y)") {
                    if (ВыгнатьВсехИзБазы) {
                        Запустить1С $ПовторАргументы $ПовторЧто
                        return
                    }
                    Write-Host "Выгнать не получилось — причина выше." -ForegroundColor Red
                }
            }
...
if ($ТолькоЗагрузка) {
    ...
    Запустить1С @("/UpdateDBCfg", $режимОбновления) "Обновление конфигурации БД" `
                @("/UpdateDBCfg", $режимОбновления) "Обновление конфигурации БД (после выгона)"
    Write-Host ""
    Write-Host "Готово: конфигурация БД обновлена." -ForegroundColor Green
} else {
    # Повтор — только обновление: загрузка из файлов к моменту ошибки блокировки уже прошла.
