...
#   powershell -File _tools\apply-config.ps1 -Спрашивать      # спросить, прежде чем выгонять людей
...
#   powershell -File _tools\apply-config.ps1 -Разблокировать  # аварийно снять блокировки после оборванного запуска
#
...
# монопольный режим. Вот тогда скрипт освобождает базу: запрещает начало сеансов,
# завершает все текущие и доделывает обновление тем же запуском (загрузку из файлов
# не повторяет, она уже прошла). По умолчанию — молча, без вопросов: с -Спрашивать
# он сначала покажет, кто работает, и спросит «выгнать?».
#
...
# Меню (Обновить базу.cmd) даёт два пункта: с вопросом (-Спрашивать) и без него.
...
param(
    ...
    [Alias("unlock")]     [switch]$Разблокировать,
    [string]$Ветка = "main"
)
...
function Запустить1С([string[]]$Аргументы, [string]$Что, [string[]]$ПовторАргументы, [string]$ПовторЧто) {
    ...
    if ($p.ExitCode -ne 0) {
        ...
        if ($текст -match "для конфигурирования") {
            ...
            Write-Host "Выгнать всех и обновить одной командой:" -ForegroundColor Yellow
            Write-Host "  apply-config.ps1 -ТолькоОбновить -ВыгнатьВсех" -ForegroundColor Yellow
        }
        ВернутьБлокировки
        Запустить1СОбратно
        exit $p.ExitCode
...
# Спрашивать можно только у живого человека за консолью: из планировщика или с
# перенаправленным вводом Read-Host вернёт пустую строку. Нужна только для -Спрашивать.
...
function ОсвободитьБазуДляПовтора {
    ...
    if ($люди.Count -eq 0) {
        ...
        if ($Спрашивать) {
            ...
            Write-Host "Выгоняю их: без монопольного режима эти правки в базу не попадут." -ForegroundColor Yellow
            Write-Host "Несохранённые данные у работающих потеряются." -ForegroundColor Yellow
        }
    }
...
function ОсвободитьБазу {
    ...
    if ($люди.Count -gt 0) {
        ...
        ПоказатьСеансы $люди
        # Монопольный режим здесь запрошен явно (-Монопольно / -ВыгнатьВсех), значит
        # выгоняем; спрашиваем только если попросили спрашивать.
        $выгонять = $true
        if ($Спрашивать -and -not $ВыгнатьВсех -and (МожноСпрашивать)) {
        ...
        if ($выгонять) {
            ...
            Write-Host "   их не трогаю — обновление может не пройти." -ForegroundColor Yellow
        }
    }
...
# Нужна, когда предыдущий запуск оборвали: закрыли окно, Ctrl+C, убили пакетный
# Конфигуратор. Скрипт до ВернутьБлокировки не дошёл, запрет начала сеансов и
# блокировка регламентных заданий остались стоять — в базу не пускает никого,
# и это никак себя не проявляет, кроме «неверный код доступа к информационной базе».
if ($Разблокировать) {
    if (-not (ПодключитьсяККластеру)) {
        Write-Host "К кластеру не подключиться — снимайте блокировки в консоли администрирования:" -ForegroundColor Red
        Write-Host "свойства ИБ -> блокировка начала сеансов и блокировка регламентных заданий." -ForegroundColor Red
        exit 1
    }
    $иб = СвойстваБазы
    if ($иб) {
        Write-Host ("   было: начало сеансов запрещено = {0}, регламентные задания запрещены = {1}" -f `
            (СвойствоCOM $иб "SessionsDenied"), (СвойствоCOM $иб "ScheduledJobsDenied")) -ForegroundColor DarkGray
    }
    if (ЗаписатьСвойстваБазы ([ordered]@{ ScheduledJobsDenied = $false; SessionsDenied = $false })) {
        Write-Host "Блокировки сняты: в базу пускают, регламентные задания работают." -ForegroundColor Green
        exit 0
    }
    Write-Host "Снять не удалось — причина выше. Консоль кластера: свойства ИБ." -ForegroundColor Red
    exit 1
}
