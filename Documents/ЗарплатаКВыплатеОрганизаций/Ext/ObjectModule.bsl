...
Процедура РасчетВыплатыСУчетомНалогов(Сотрудники ) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		РасчитатьСуммыСотрудника( Выборка.Сумма,  СтрокаРаботники, СтруктураКурса, мОкруглятьДо);
...
Процедура РасчитатьСуммыСотрудника( НоваяСумма,  СтрокаРаботники, СтруктураКурса, мОкруглятьДо) Экспорт
	...
	Если мОкруглятьДо > 0.01 И ВидВыплаты <> Справочники.ВидыВыплат.АвансПредварительныйРасчет Тогда
...
Процедура Печать(НазваниеМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект,"Платежная ведомость"),Ссылка);
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	Если ВидВыплаты = ВидыВыплат.ОчереднаяВзносы ИЛИ ВидВыплаты = ВидыВыплат.ОчереднаяПогашениеДолговВзносы ИЛИ ВидВыплаты = ВидыВыплат.НДФЛКонтрагентам ИЛИ ВидВыплаты = ВидыВыплат.НДФЛБезВыплатыЗарплаты Тогда
		Возврат Новый Структура("АвансНалоги","Налоги и взносы");	
	Иначе
		Возврат Новый Структура("Ведомость, ВедомостьКасса, ВедомостьБанк,ВедомостьДо2009, ВедомостьКассаДо2009, ВедомостьБанкДо2009,АвансНалоги",
