...
Функция ПечатьИнв17() Экспорт
	...
	Шапка.Параметры.МесяцДокумента = Сред(Формат(Выборка.Дата, "ДЛФ=ДД;"+НСтр("ru='Л=ru';uk='Л=ru'", КодЯзыкаПечать)), 
	                                   Найти(Формат(Выборка.Дата, "ДЛФ=ДД;"+НСтр("ru='Л=ru';uk='Л=ru'",КодЯзыкаПечать)), " "));
													 
	Шапка.Параметры.ДеньОснования = День(Выборка.ДокументОснованиеДата);
	Шапка.Параметры.МесяцОснования = Сред(Формат(Выборка.ДокументОснованиеДата, "ДЛФ=ДД;"+НСтр("ru='Л=ru';uk='Л=ru'", КодЯзыкаПечать)), 
	                                   Найти(Формат(Выборка.ДокументОснованиеДата, "ДЛФ=ДД;"+НСтр("ru='Л=ru';uk='Л=ru'", КодЯзыкаПечать)), " "));
	...
	Если ВыборкаПоВидуЗадолженности.НайтиСледующий(Перечисления.ВидыЗадолженности.Дебиторская, "ВидЗадолженности") Тогда
		...
		Пока ВыборкаПоКонтрагенту.Следующий() Цикл
			...
			СтрокаТаблицыДт.Параметры.КонтрагентПредставление = ?(ПустаяСтрока(ВыборкаПоКонтрагенту.КонтрагентНаименованиеПолное),
				ВыборкаПоКонтрагенту.Наименование, ВыборкаПоКонтрагенту.КонтрагентНаименованиеПолное);
