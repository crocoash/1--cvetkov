...
Функция ОпределитьВидКорректировкиНК(Данные) Экспорт
	...
	НалоговыйКредитВыход = Данные.НалоговоеНазначение = Справочники.НалоговыеНазначенияАктивовИЗатрат.НДС_Пропорционально ИЛИ УчетНДС.ЕстьПравоНаНалоговыйКредит(Данные.НалоговоеНазначениеНовое);
...
Функция ПодготовитьТаблицуТоваров(РезультатЗапросаПоТоварам, СтруктураШапкиДокумента)
	...
	Для каждого СтрокаТаблицы Из ТаблицаТоваров Цикл
		...
		СтрокаТаблицы.БазаНДС  = СтрокаТаблицы.НДС*100/УчетНДС.ПолучитьСтавкуНДС(Перечисления.СтавкиНДС.НДС20);
...
Процедура ПроверитьЗаполнениеШапки(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Если  СтруктураШапкиДокумента.ВидСклада = Перечисления.ВидыСкладов.НТТ
		...
		ТекстСообщения = "Нельзя изменять налоговое назначение запасов в неавтоматизированной торговой точке "
"если применяется оценка товаров в рознице ""по продажным ценам!""";
...
Процедура ПроверитьЗаполнениеМетодКорректировкиНалоговогоКредитаВТабличнойЧасти(Таблица, ИмяТабличнойЧасти, Отказ, Заголовок)
	...
	Для каждого СтрокаТЧ  Из Таблица Цикл
		...
		СтрокаНачалаСообщенияОбОшибке = Локализация.СтрШаблон1("В строке номер ""¤1¤"": ", СокрЛП(СтрокаТЧ.НомерСтроки));
...
Процедура ПроверитьВозможностьКорректировки(Таблица, ИмяТабличнойЧасти, Отказ, Заголовок)
	...
	Для каждого СтрокаТЧ  Из Таблица Цикл
		...
		СтрокаНачалаСообщенияОбОшибке = Локализация.СтрШаблон1(НСтр("ru='В строке номер ""¤1¤"": ';uk='У рядку номер ""¤1¤"": '"), СокрЛП(СтрокаТЧ.НомерСтроки));
...
Процедура ДвиженияПоПартиям(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварам, Отказ, Заголовок)
	...
	Если глЗначениеПеременной("ИспользоватьУправляемыеБлокировки") Тогда
		...
		СтруктураПараметровБлокировки.Вставить("ИсточникДанных", ТаблицаПоТоварам);
	...
	Для каждого СтрокаТаблицы Из ТаблицаПоТоварам Цикл
		...
		Если КоличествоОсталосьСписать >0 Тогда
			...
			ОбщегоНазначения.СообщитьОбОшибке(Локализация.СтрШаблон1(НСтр("ru = 'Номер строки: ¤1¤. Не хватает ¤2¤ ¤3¤ товара ¤4¤, счет учета ¤5¤, налоговое назначение (НДС): ¤6¤'; uk = 'Номер рядка: ¤1¤ Не вистачає ¤2¤ ¤3¤ товари ¤4¤, рахунок ¤5¤ , податкове призначення (ПДВ): ¤6¤'"), СтрокаТаблицы.НомерСтроки, КоличествоОсталосьСписать, СтрокаТаблицы.Номенклатура.БазоваяЕдиницаИзмерения, СтрокаТаблицы.Номенклатура, СтрокаТаблицы.СчетУчетаБУ, СтрокаТаблицы.НалоговоеНазначение), "");
