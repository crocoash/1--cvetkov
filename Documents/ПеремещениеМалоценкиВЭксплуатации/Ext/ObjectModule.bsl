...
Функция ПроверкаРеквизитов(Отказ, Заголовок) Экспорт
	...
	Если ОтражатьВБухгалтерскомУчете Тогда
		РеквизитыШапки = РеквизитыШапки + ", Организация, ПодразделениеОрганизации";
	КонецЕсли;	
...
Процедура ПроверитьНазначенияИспользования(СтруктураШапкиДокумента, ТаблицаПоМалоценке, Отказ, Заголовок)
	...
	Для каждого СтрокаТЧ Из ТаблицаПоМалоценке Цикл
		...
		СтрокаНачалаСообщенияОбОшибке = Локализация.СтрШаблон1("Строка номер ""¤1¤"" табличной части ""малоценные активы"": ", СокрЛП(СтрокаТЧ.НомерСтроки)) + Символы.ПС;
		...
		Если СтрокаТЧ.Номенклатура <> СтрокаТЧ.ВладелецНазначения Тогда
			...
			ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + Символы.Таб
			                 + Локализация.СтрШаблон1("Назначение использования <¤1¤> не соответствует номенклатуре <¤2¤>.", СтрокаТЧ.НазначениеИспользования, СтрокаТЧ.Номенклатура),
							 Отказ, Заголовок);
		...
		Если СтрокаТЧ.Номенклатура <> СтрокаТЧ.ВладелецНазначенияНового Тогда
			...
			ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + Символы.Таб
			                 + Локализация.СтрШаблон1("Новое назначение использования <¤1¤> не соответствует номенклатуре <¤2¤>.", СтрокаТЧ.НазначениеИспользованияНовое, СтрокаТЧ.Номенклатура),
							 Отказ, Заголовок);
			СтрокаНачалаСообщенияОбОшибке = "";
		...
		Если НЕ ЗначениеЗаполнено(СтрокаТЧ.СпособОтраженияРасходов) Тогда
			...
			ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + Символы.Таб
							 + Локализация.СтрШаблон1("В назначении использования <¤1¤> не выбран ""Способ отражения расходов"".", СтрокаТЧ.НазначениеИспользования),
							 Отказ, Заголовок);
			СтрокаНачалаСообщенияОбОшибке = "";
		...
		Если НЕ ЗначениеЗаполнено(СтрокаТЧ.СпособОтраженияРасходовНовый) Тогда
			...
			ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + Символы.Таб
							 + Локализация.СтрШаблон1("В новом назначении использования <¤1¤> не выбран ""Способ отражения расходов"".", СтрокаТЧ.НазначениеИспользованияНовое),
							 Отказ, Заголовок);
			СтрокаНачалаСообщенияОбОшибке = "";
		...
		Если НЕ ЗначениеЗаполнено(СтрокаТЧ.ВидМалоценки) Тогда
			...
			ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + Символы.Таб
			                 + Локализация.СтрШаблон1("В назначении использования <¤1¤> не выбран ""Вид малоценки"".", СтрокаТЧ.НазначениеИспользования),
			                 Отказ, Заголовок);
			СтрокаНачалаСообщенияОбОшибке = "";
		...
		Если НЕ ЗначениеЗаполнено(СтрокаТЧ.ВидМалоценкиНовый) Тогда
			...
			ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + Символы.Таб
			                 + Локализация.СтрШаблон1("В новом назначении использования <¤1¤> не выбран ""Вид малоценки"".", СтрокаТЧ.НазначениеИспользования),
			                 Отказ, Заголовок);
			СтрокаНачалаСообщенияОбОшибке = "";
		...
		Иначе
				...
				Если СтрокаТЧ.ВидМалоценки <> СтрокаТЧ.ВидМалоценкиНовый Тогда
					...
					ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + Символы.ПС + Символы.Таб
					                 + Локализация.СтрШаблон1("Вид малоценки нового назначения использования <¤1¤> не соответствует виду малоценки старого назначения использования <¤2¤>.", СтрокаТЧ.ВидМалоценкиНовый, СтрокаТЧ.ВидМалоценки),
									 Отказ, Заголовок);
				СтрокаНачалаСообщенияОбОшибке = "";
		...
		Если ОтражатьВБухгалтерскомУчете Тогда
			...
			Если НЕ ЗначениеЗаполнено(СтрокаТЧ.СчетПередачиБУ) Тогда
				...
				ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + Символы.Таб
				                 + Локализация.СтрШаблон1("В назначении использования <¤1¤> не выбран ""Счет передачи"".", СтрокаТЧ.НазначениеИспользования),
				                 Отказ, Заголовок);
				СтрокаНачалаСообщенияОбОшибке = "";
			...
			Если НЕ ЗначениеЗаполнено(СтрокаТЧ.СчетПередачиБУНовый) Тогда
				...
				ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + Символы.Таб
				                 + Локализация.СтрШаблон1("В новом назначении использования <¤1¤> не выбран ""Счет передачи"".", СтрокаТЧ.НазначениеИспользованияНовое),
				                 Отказ, Заголовок);
				СтрокаНачалаСообщенияОбОшибке = "";
			...
			Если (СтрокаТЧ.ВидМалоценки <> мМБП) Тогда
				...
				Если НЕ ЗначениеЗаполнено(СтрокаТЧ.СчетАмортизацииБУ) Тогда
					...
					ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + Символы.Таб
					                 + Локализация.СтрШаблон1("В назначении использования <¤1¤> не выбран ""Счет амортизации"".", СтрокаТЧ.НазначениеИспользования),
					                 Отказ, Заголовок);
					СтрокаНачалаСообщенияОбОшибке = "";
				...
				Если НЕ ЗначениеЗаполнено(СтрокаТЧ.СчетАмортизацииБУНовый) Тогда
					...
					ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + Символы.Таб
					                 + Локализация.СтрШаблон1("В новом назначении использования <¤1¤> не выбран ""Счет амортизации"".", СтрокаТЧ.НазначениеИспользованияНовое),
					                 Отказ, Заголовок);
					СтрокаНачалаСообщенияОбОшибке = "";
			...
			Если СтруктураШапкиДокумента.ЕстьНДС ИЛИ СтруктураШапкиДокумента.ЕстьНалогНаПрибыль Тогда
				...
				Если НЕ ЗначениеЗаполнено(СтрокаТЧ.НалоговоеНазначение) Тогда
					...
					ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + Символы.Таб
									 + Локализация.СтрШаблон1("Не заполнено ""Налоговое назначение""."),
									 Отказ, Заголовок);
					СтрокаНачалаСообщенияОбОшибке = "";
