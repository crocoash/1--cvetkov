	...
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
		Для Каждого Строка Из Комплектующие Цикл
			Если Строка.СчетУчетаБУ.Забалансовый Тогда
				ОбщегоНазначения.СообщитьОбОшибке("В строке номер """ + СокрЛП(Строка.НомерСтроки) + """ табличной части ""Комплектующие"" содержится товар, учитываемый на забалансовом счете. Таких здесь быть не должно!", Отказ, Заголовок);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеТаблицыПоКомплектам(ТаблицаПоКомплектующим, СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Если СтруктураШапкиДокумента.ВидСклада = Перечисления.ВидыСкладов.Розничный
		...
		ОбщегоНазначения.СообщитьОбОшибке("Для """ + СокрЛП(ТаблицаПоКомплектующим[0].Номенклатура) + "Не задана цена АТТ." + Символы.ПС + "Необходимо выполнить переоценку!", Отказ, Заголовок);	
