		...
		Пока Выборка.СледующийПоЗначениюПоля("Сотрудник") Цикл
			Пока Выборка.СледующийПоЗначениюПоля("Назначение") Цикл
				...
				Если НЕ Выборка.ВведенныйДокументВЦеломЗаПериод = NULL Тогда
					Сообщить("Сотруднику - "+Выборка.Назначение+" введен табель "+ Выборка.ВведенныйДокументВЦеломЗаПериод +" в целом за период, поэтому невозможно заполнить табель по дням.");
					Продолжить;
				КонецЕсли;
				...
				Если ВведенныеТабеляПоДнямПоСотруднику.Количество() > 0 Тогда
					ТекстСообщения = "Сотруднику - "+Выборка.Назначение+" уже введены документы ";
					Для Каждого ВведенныйТабель Из ВведенныеТабеляПоДнямПоСотруднику Цикл
						ТекстСообщения = ТекстСообщения + ВведенныйТабель.Ключ+", ";
					КонецЦикла;
					ТекстСообщения = ТекстСообщения + " поэтому не все даты периода заполнены.";
...
Функция ПроверкаУникальностиЗаписи()
	...
	Если ВыборкаРезультата.Количество() > 0 Тогда
		...
		ОбщегоНазначения.ОшибкаПриПроведении("Проведение документа невозможно так как на следующих сотрудников уже введены данные в этом периоде" + СтрокаСотрудников,Истина);
		Возврат Ложь;
	Иначе
		Возврат Истина;
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеШапки(ВыборкаПоШапкеДокумента, Отказ, Заголовок = "")
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.Организация) Тогда
		ОбщегоНазначения.ОшибкаПриПроведении("Не указана организация, по которой вводится табель!", Отказ, Заголовок);
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеСтрокиОтработанноеВремя(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ, Заголовок = "")
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.Назначение) Тогда
		ОбщегоНазначения.ОшибкаПриПроведении(СтрокаНачалаСообщенияОбОшибке + "не выбрано назначение!", Отказ, Заголовок);
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеСтрокиОтработанноеВремяВЦеломЗаПериод(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ, Заголовок = "")
	...
	// Назначение
	Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.Назначение) Тогда
		ОбщегоНазначения.ОшибкаПриПроведении(СтрокаНачалаСообщенияОбОшибке + "не указано назначение!", Отказ, Заголовок);
	КонецЕсли;
