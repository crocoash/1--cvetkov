...
Функция ЗаполнитьТаблицуРасчетногоЛистка(ТекущийСотрудник) Экспорт
	...
	Если ДобавитьДанныеИзДокумента Тогда
		...
		Иначе
			...
			СуммаНаНачалоПериода = 0;
		КонецЕсли; 			
		ТекстПериода = Формат(ПериодРегистрации, НСтр("ru='ДФ = 'ММММ гггг ''г.''''"));
		
		// Очистим временный табличный документ
		ВременныйТабличныйДокумент.Очистить();
		
		ОбластьШапка.Параметры.Работник = НСтр("ru='Работник: ';uk='Працівник: '",Локализация.КодЯзыкаИнформационнойБазы()) + ФИОФизлица;
		ОбластьШапка.Параметры.КодПоДРФО = НСтр("ru='Код по ДРФО: ';uk='Код за ДРФО: '",Локализация.КодЯзыкаИнформационнойБазы()) + ТекущийСотрудник.ФизЛицо.КодПоДРФО; 
		...
		Для Каждого СтрокаНачислений Из Строки Цикл
			...
			Если СтрокаНачислений.Назначение.ОсновноеНазначение = СтрокаНачислений.Сотрудник Тогда
				мВидЗанятости = НСтр("ru=' (по внутр. совмест-ву)';uk=' (по внутр. сумуст-ву)'",Локализация.КодЯзыкаИнформационнойБазы())
			КонецЕсли;
		...
		Для Каждого СтрокаНДФЛ Из СтрокиНДФЛ Цикл
			...
			СтрокаТаблицы.ВидДвижения			= 3;
			СтрокаТаблицы.ВидРасчета			= НСтр("ru='НДФЛ исчисленный';uk='ПДФО обчислений'",Локализация.КодЯзыкаИнформационнойБазы());
		...
		Если СуммаНаНачалоПериода >= 0  Тогда
			ОбластьПодвал.Параметры.ТекстДолгНаНачалоПериода = НСтр("ru='Долг за организацией на начало месяца: ';uk='Борг за організацією на початок місяця: '",Локализация.КодЯзыкаИнформационнойБазы());
			ОбластьПодвал.Параметры.СуммаДолгНаНачалоПериода = СуммаНаНачалоПериода;
		Иначе
			ОбластьПодвал.Параметры.ТекстДолгНаНачалоПериода = НСтр("ru='Долг за работником на начало месяца: ';uk='Борг за працівником на початок місяця: '",Локализация.КодЯзыкаИнформационнойБазы());
			ОбластьПодвал.Параметры.СуммаДолгНаНачалоПериода = 0 - СуммаНаНачалоПериода;
		КонецЕсли;
				
		Если СуммаНаКонецПериода >= 0  Тогда
			ОбластьПодвал.Параметры.ТекстДолгНаКонецПериода = НСтр("ru='Долг за организацией на конец месяца: ';uk='Борг за організацією на кінець місяця: '",Локализация.КодЯзыкаИнформационнойБазы());
			ОбластьПодвал.Параметры.СуммаДолгНаКонецПериода  = СуммаНаКонецПериода;
		Иначе
			ОбластьПодвал.Параметры.ТекстДолгНаКонецПериода = НСтр("ru='Долг за работником на конец месяца: ';uk='Борг за працівником на кінець місяця: '",Локализация.КодЯзыкаИнформационнойБазы());
			...
			Пока ВыборкаСотрудники.Следующий() Цикл
				...
				Пока Выборка.Следующий() Цикл
					...
					Если НЕ ЗначениеЗаполнено(Выборка.СтатьяЗатрат)Тогда
						Если Выборка.ВидРасчетаБазы.ЭтоБольничные И ЗначениеЗаполнено(Выборка.СчетДт) Тогда
				...
				Иначе
					...
					Иначе
						...
						ИначеЕсли ПриходСумма > 0 
							...
							РасходСуммаУпр = Мин(ПриходСуммаУпр, Выборка.РасходСуммаДокументаУпр);
...
Процедура ПроверитьЗаполнениеСтрокиНачисления( ВыборкаПоСтрокамДокумента, Отказ, Заголовок = "")
	...
	Если ОтражатьВРеглУчете Тогда
		...
		ИначеЕсли ВыборкаПоСтрокамДокумента.Сотрудник.ВидДоговора = Перечисления.ВидыДоговоровСФизЛицами.ТрудовойДоговор
			И ВыборкаПоСтрокамДокумента.Базовыйпериодначало  < ВыборкаПоСтрокамДокумента.ДатаПриема Тогда	
			ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru=' дата начала базового периода начисления меньше даты приема!';uk='дата початку базового періоду нарахування меньша за дату прийому!'"), Отказ, Заголовок);
...
Процедура ПроверитьЗаполнениеСтрокиОтраженияВУчете( ВыборкаПоДоходам, Отказ, Заголовок = "" )
	...
	СтрокаНачалаСообщенияОбОшибке = Локализация.СтрШаблон1(НСтр("ru='В строке номер ""¤1¤"" табл. части ""Проводки"": ';uk='У рядку номер ""¤1¤"" табл. частини ""Проводки"": '"), СокрЛП(ВыборкаПоДоходам.НомерСтроки));
...
Процедура ДобавитьСтрокуВДвиженияПоОтражениюВУчете( СтруктураШапкиДокумента, ВыборкаПоДоходам, ВалютаРеглУчета )
	...
	Если Проводка.СчетДт.НалоговыйУчет Тогда
		...
		ИначеЕсли ХарактерЗатрат = "Затраты" И СтруктураШапкиДокумента.ЕстьНалогНаПрибыль Тогда
		...
		ИначеЕсли ХарактерЗатрат = "РБП" Тогда
			...
			ИначеЕсли ХарактерЗатратРБП = "Затраты" И СтруктураШапкиДокумента.ЕстьНалогНаПрибыль Тогда
