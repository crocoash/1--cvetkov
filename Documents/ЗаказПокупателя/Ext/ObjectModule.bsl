...
// Приводит строки розничного заказа к цене, которую покупатель видит на сайте.
//
// «Цены и валюта» -> «Перезаполнить цены» ставит прайс типа цен, а скидку витрины
// (у зимней коллекции «-19%» - уточняющий процент наценки) 1С считает только в
// выгрузке на сайт: заказ 28270 после перезаполнения стоял по 3100 при цене сайта
// 2520. Цена сайта заводится через сумму строки, как при загрузке заказа с сайта
// (ОбменПокупателямиНашСайт): цена остаётся прайсовой, разница уходит в «% Руч.».
//
// Только розничный тип цен сайта (000000020) и гривневый документ. Наборы-ростовки
// не трогаются - их цену собирает ПересчитатьЦенуНК формы по комплектующим.
// Позиция, которой нет в выгрузке сайта, остаётся по прайсу.
//
Процедура ЗаполнитьЦеныВитриныСайта() Экспорт

	Если Не ЗначениеЗаполнено(ТипЦен) Или ТипЦен.Код <> "000000020" Тогда
		Возврат;
	КонецЕсли;
	...
	ЦеныСайта = ОбщегоНазначения.ЦеныВитриныСайта(МассивНоменклатуры);
	...
	Для Каждого СтрокаТЧ Из Товары Цикл
		...
		ЦенаСайта = ЦеныСайта.Получить(СтрокаТЧ.ХарактеристикаНоменклатуры);

		Если ЦенаСайта = Неопределено Или СтрокаТЧ.Количество = 0 Тогда
			Продолжить;
		КонецЕсли;

		ЦенаЗаписанаПрямо = (СтрокаТЧ.Цена = 0);

		Если Не ЦенаЗаписанаПрямо Тогда

			СтрокаТЧ.Сумма = ЦенаСайта * СтрокаТЧ.Количество;

			// Разрядность ПроцентСкидкиНаценки - 5 знаков: если прайс отличается от
			// цены сайта больше чем на порядок, процент не влезет - пишем цену прямо.
			Попытка
				ОбработкаТабличныхЧастей.ПриИзмененииСуммыТабЧасти(
					СтрокаТЧ, ЭтотОбъект, Неопределено, Ложь, Истина, Истина, Истина, "Товары");
			Исключение
				ЦенаЗаписанаПрямо = Истина;
			КонецПопытки;

		КонецЕсли;

		Если ЦенаЗаписанаПрямо Тогда
			СтрокаТЧ.Цена                        = ЦенаСайта;
...
// УБРАНО (2026-08-11): здесь запускалось фоновое задание
// "БИАС.ВыгрузитьЗаказИзОчередиНаСайт" с ключом = идентификатор заказа, а отказ по
// занятому ключу гасился Попыткой как "штатная ситуация". Штатной она не была:
// ФоновыеЗадания.Выполнить обращается к базе, и любая его ошибка ВНУТРИ транзакции
// записи документа помечает транзакцию сбойной. Попытка ловит текст ошибки, но
// транзакцию уже не спасает - следующее же обращение к базе (подписка
// ИзменениСтатусаЗаказаПриЗаписи, РегистрыСведений...ПолучитьПоследнее) падает с
// "В данной транзакции уже происходили ошибки!", и весь заказ не записывается.
// Ключ занимался штатно: групповые операции (рассылка ТТН) пишут один и тот же
// заказ дважды подряд, пока первое задание ещё живо.
// Ускорение не потеряно: заказ уже стоит в очереди ЗаказыКВыгрузкеНаСайт строкой
// выше, её разгребает регламентное задание ВыгрузкаЗаказовНаСайт.
