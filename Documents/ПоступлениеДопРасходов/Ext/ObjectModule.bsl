...
Процедура Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь, НепосредственнаяПечать = Ложь) Экспорт
	...
	УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект, "Поступление доп. расходов"), Ссылка);
...
Процедура ДобавитьРаспределениеУпр( ТаблицаПоТоварам,  ТаблицаПоОборудованию, СтруктураШапкиДокумента)
	...
	Если СтруктураШапкиДокумента.ОтражатьВУправленческомУчете Тогда
		...
		Для К = СтрокТоваров + 1 По СтрокТоваров + ТаблицаПоОборудованию.Количество()  Цикл
			...
			ТаблицаПоОборудованию[К-СтрокТоваров-1].СуммаВзаиморасчетовРаспределения  = МассивВзаиморасчетов[К-СтрокТоваров-1];
	...
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
		Для Каждого СтрокаТЧ из ТаблицаПоТоварам Цикл
			...
			Если ЗначениеЗаполнено(СтрокаТЧ.СчетУчетаБУ) И СтрокаТЧ.СчетУчетаБУЗабалансовый Тогда
				...
				СтрокаНачалаСообщенияОбОшибке = Локализация.СтрШаблон1("Строка номер ""¤1¤"" табличной части ""Товары"": Доп. расходы не могут быть отнесены на забалансовый счет!", СокрЛП(СтрокаТЧ.НомерСтроки)); 
...
Процедура РаспределитьПоСкладамУпр(ТаблицаПоТоварам, ТаблицаРаспределенияПоСкладам, СтруктураШапкиДокумента, СтатусПартии, Отказ, Заголовок)
	...
	Для Каждого Строка Из ТаблицаПоТоварам Цикл
		...
		Если МассивСтрокРаспределения.Количество() = 0 тогда
			...
			+ ?(Строка.Номенклатура.ВестиУчетПоХарактеристикам, ", х-ка: " + Строка.ХарактеристикаНоменклатуры, "")
			+ ?(Строка.Номенклатура.ВестиУчетПоСериям, ", серия: " + Строка.СерияНоменклатуры, "")
			+ ?(ОтражатьВБухгалтерскомУчете, ", нал. назн.: " + Строка.НалоговоеНазначение, "");
			ОбщегоНазначения.СообщитьОбОшибке(ТекстСообщения, Отказ, Заголовок);
		КонецЕсли;
