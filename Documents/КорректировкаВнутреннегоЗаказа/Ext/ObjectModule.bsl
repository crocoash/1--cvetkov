	...
	Для Каждого СтрокаТЧ Из ТаблицаПоТоварам Цикл
		Если ТипЗнч(СтрокаТЧ.Размещение) = Тип("СправочникСсылка.Склады") Тогда
			Если СтрокаТЧ.Размещение.ВидСклада = Перечисления.ВидыСкладов.НТТ Тогда
				ОбщегоНазначения.ОшибкаПриПроведении("В строке № " + СтрокаТЧ.НомерСтроки + " табличной части """ + ИмяТабличнойЧасти + """ выбрана неавтоматизированная торговая точка. " + Символы.ПС + "Нельзя выбирать неавтоматизированные торговые точки.", Отказ, Заголовок);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого СтрокаТЧ Из ТаблицаПоТоварам Цикл
		Если ТипЗнч(СтрокаТЧ.Размещение) = Тип("СправочникСсылка.Склады")
			...
			Если СтрокаТЧ.Размещение = ВнутреннийЗаказ.Заказчик Тогда
				ОбщегоНазначения.ОшибкаПриПроведении("В строке № " + СтрокаТЧ.НомерСтроки + " табличной части """ + ИмяТабличнойЧасти + """ выбран склад, совпадающий со складом-заказчиком внутреннего заказа. " + Символы.ПС + "Нельзя зарезервировать товар на складе-заказчике.", Отказ, Заголовок);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
