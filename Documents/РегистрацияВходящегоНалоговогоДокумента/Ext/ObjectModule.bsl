			...
			Если  Строка.ПропорциональныйНДС Тогда
				...
				Если НЕ Строка.СтавкаНДС = Перечисления.СтавкиНДС.НДС20
				   ...
				   И (Строка.СтатьяДекларацииНДСНалоговыйКредит = Справочники.СтатьиНалоговыхДеклараций.НДС_НКПропорционально ИЛИ
					...
					СтрокаСообщения = Локализация.СтрШаблон1("В строке ""¤1¤"" указан признак пропорционального НДС для ставки НДС отличной от 20%(0%)! Статья декларации НДС(н/к) в этом случае не должна входить в группу ""¤2¤""!.", Строка.НомерСтроки, Справочники.СтатьиНалоговыхДеклараций.НДС_НКПропорционально);
				...
				ИначеЕсли НЕ Строка.ДляХозяйственнойДеятельности Тогда
					...
					СтрокаСообщения = Локализация.СтрШаблон1("В строке ""¤1¤"" указан признак пропорционального НДС! В этом случае должен быть установлен признак использования в хозяйственной деятельности!.", Строка.НомерСтроки);
					ОбщегоНазначения.ОшибкаПриПроведении(СтрокаСообщения, Отказ, Заголовок);				
...
Процедура ДвиженияПоРегистрам(СтруктураШапкиДокумента, ТаблицаПоТоварам, Отказ, Заголовок);
	...
	ТаблицаДвижений.Свернуть("","ПроводкиСуммаНДС");
...
Процедура ОбработкаЗаполнения(Основание)
	...
	Если  ТипЗнч(Основание) = Тип("ДокументСсылка.КорректировкаОжидаемогоИПодтвержденногоНДС")
	  ИЛИ ТипЗнч(Основание) = Тип("ДокументСсылка.ИзменениеНалоговогоНазначенияЗапасов") 
