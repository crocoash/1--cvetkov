...
Процедура Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь, НепосредственнаяПечать = Ложь) Экспорт
	...
	УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект,"Увольнение "), Ссылка);
	...
	Если ВыборкаПоСтрокамДокумента.ВидСтрокиЗапроса = "ДанныеДляДвиженийКадров" Тогда
		...
		Если ВыборкаПоШапкеДокумента.ОтражатьВРеглУчете Тогда
			...
			ИначеЕсли ВыборкаПоСтрокамДокумента.ОшибкаВПриказеОПриемеНетФизЛица Тогда
				ОбщегоНазначения.ОшибкаПриПроведении(СтрокаНачалаСообщенияОбОшибке + " работник и приказ о приеме не соответствуют друг другу!", Отказ);
			КонецЕсли;
		КонецЕсли;
				...
				Пока Выборка.Следующий() Цикл
					Если Выборка.Актуальность = Истина Тогда
						...
						Если Выборка.Период = НачалоМесяца(ВыборкаПоРаботникиОрганизации.ДатаУвольнения) Тогда
							Сообщить(Локализация.СтрШаблон1("В данном месяце работнику ¤1¤ назначена ¤2¤."
"Перед проведением документа удалите данную льготу.", Выборка.Сотрудник, Выборка.Льгота.Наименование), СтатусСообщения.Важное);
							мОтказ = Истина;
							Прервать;
						КонецЕсли;
