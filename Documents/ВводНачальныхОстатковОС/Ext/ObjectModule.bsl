		...
		Для Каждого Колонка Из СтруктРекв Цикл
			...
			Если Не Колонка.Ключ = "ОсновноеСредство" Тогда
				...
				Если ТипЗначения = Тип("Булево") Тогда
					...
					Значение = Формат(Значение, НСтр("ru='БЛ=Нет; БИ=Да';uk='БЛ=Немає; БИ=Так'", КодЯзыкаПечать));
...
Процедура ПроверитьЗаполнениеШапки(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Если ОтражатьВБухгалтерскомУчете Тогда
		...
		Если ВводитьОСВременнаяНалоговаяРазница Тогда
			СтрРекв = СтрРекв + ", ОсновноеСредствоВременнаяНалоговаяРазница, СтоимостьНУВременнаяНалоговаяРазница";
		КонецЕсли;	
...
Функция ПроверкаПоСтроке(СтрокаОС,БезусловныеРеквизиты, БухУчет, НалУчет, НалУчетОбщ, Отказ, Заголовок,МетаданныеРеквизиты)
	...
	Если НалУчет И СтрокаОС.НачислятьАмортизациюБУ Тогда                                                                                                                                               
		УправлениеВнеоборотнымиАктивами.ПроверитьСоответствиеНалоговогоНазначенияАмортизацииОбъекта(СтрокаОС.ОсновноеСредство, СтрокаОС.СпособОтраженияРасходовПоАмортизацииБУ, СтрокаОС.НалоговоеНазначение, Отказ, Заголовок, Дата);
	КонецЕсли;
...
Процедура ОшибкаПроведенияПоСтроке(СтрокаОС, ТекстСообщения,Заголовок,Статус)
	НачалоСообщения = Локализация.СтрШаблон1("- строка № ¤1¤, инв. номер ОС <¤2¤> : ", СтрокаОС.НомерСтроки, СтрокаОС.ИнвентарныйНомерРегл);
