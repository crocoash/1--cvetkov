...
Процедура Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь, НепосредственнаяПечать = Ложь) Экспорт
	...
	УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект, "Ремонт ОС"), Ссылка);
...
Процедура ПроверкаРеквизитовТЧУпр(ТаблицаОС, СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Если ТаблицаОС.Итог("СуммаРемонтаУУ")<> СтруктураШапкиДокумента.СтоимостьУУ Тогда
		...
		ОбщегоНазначения.ОшибкаПриПроведении("По управленческому учету общая сумма ремонта, указанная в шапке документа, не соответствует в итоге суммам, отнесенным на основные средства!", Отказ, Заголовок);
...
Процедура ПроверкаРеквизитовТЧРегл(ТаблицаОС, СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Если ТаблицаОС.Итог("СуммаРемонтаБУ")<> СтруктураШапкиДокумента.СтоимостьБУ Тогда
		...
		ОбщегоНазначения.ОшибкаПриПроведении("По бухгалтерскому учету общая сумма ремонта, указанная в шапке документа, не соответствует в итоге суммам, отнесенным на основные средства!", Отказ, Заголовок);
	...
	Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете 
		...
		Если ТаблицаОС.Итог("СуммаРемонтаНУ")<> СтруктураШапкиДокумента.СтоимостьНУ Тогда
			...
			ОбщегоНазначения.ОшибкаПриПроведении("По налоговому учету общая сумма ремонта, указанная в шапке документа, не соответствует в итоге суммам, отнесенным на основные средства!", Отказ, Заголовок);
...
"	Ремонт в этом случае невозможна.", СтрокаТЧ.НомерСтроки, СтрокаТЧ.ВидНалоговойДеятельности_ОС, СтрокаТЧ.НалоговоеНазначение_ОС, ТекОС, ВидНалоговойДеятельности, НалоговоеНазначениеОбъектаСтр),
										Отказ, Заголовок);
...
"	налогового назначения объекта строительства <¤6¤>."
"	Ремонт в этом случае невозможна.", СтрокаТЧ.НомерСтроки, СтрокаТЧ.ВидДеятельностиНДС_ОС, СтрокаТЧ.НалоговоеНазначение_ОС, ТекОС, ВидДеятельностиНДС, НалоговоеНазначениеОбъектаСтр),
										Отказ, Заголовок);
			...
			Если (СтрокаТЧ.УчитываетсяКакЗапасНУ И НЕ СтрокаТЧ.НеПроизводственноеОСНУ) ИЛИ СтрокаТЧ.НеУчитываетсяКакОСНУ Тогда
				...
				Если НЕ ЗначениеЗаполнено(СтруктураШапкиДокумента.СтатьяВаловыхРасходов) Тогда
					...
					Если СтрокаТЧ.УчитываетсяКакЗапасНУ Тогда
						ТекстСообщенияОбОшибке = Локализация.СтрШаблон1("Строка ¤1¤."
"	Основное средство ""¤2¤"" учитывается как запас в налоговом учете."
"	Ремонт в этом случае относится на валовые расходы, а статья не указана.", СтрокаТЧ.НомерСтроки, ТекОс);
					Иначе
						ТекстСообщенияОбОшибке = Локализация.СтрШаблон1("Строка ¤1¤."
