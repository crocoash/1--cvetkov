...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект,"Начисление больничного "),Ссылка);
...
Функция РассчитатьСреднююПорядок1266( КомментироватьРасчет = Ложь ) Экспорт
	...
	Если КомментироватьРасчет Тогда
		...
		КомментарийРасчетаСредней = ПроведениеРасчетов.КомментарийРасчета("Расчет средней с " + Формат(ПериодРасчетаСреднегоНачало,"ДФ='МММ гггг'")+ " по "+
	...
	Пока Выборка.Следующий() Цикл
		...
		Если КомментироватьРасчет Тогда
			...
			КомментарийНомерСтроки = ПроведениеРасчетов.КомментарийРасчета("Номер строки: " + Выборка.НомерСтроки, КомментарийРасчетаСредней,,,,Выборка.Результат <> 0);
			КомментарийВидаРасчета = ПроведениеРасчетов.КомментарийРасчета("	Вид расчета: " +  СтрокаТабличнойЧасти.ВидРасчета, КомментарийНомерСтроки);
			ПроведениеРасчетов.КомментарийРасчета("	Базовый период: " +Формат(Выборка.БазовыйПериодНачало,"ДФ='МММ гггг'"), КомментарийВидаРасчета);
			КомментарийБазовыйПериод = ПроведениеРасчетов.КомментарийРасчета("	Базовые начисления составили: " + ОКР(Выборка.РезультатПолный,2,0), КомментарийВидаРасчета,,,,Ложь);
...
Процедура ПроверитьЗаполнениеСтрокиНачисления(ВыборкаПоСтрокамДокумента, Отказ, Заголовок = "")
	...
	Если Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ГрафикРаботы) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "не указан график работы!", Отказ, Заголовок);
	КонецЕсли;
