...
//
Процедура ПечатьЭтикеток(Товары = Неопределено, Большая = Ложь, Режим = Неопределено) Экспорт
	
	Если Режим = "Процентовка" Тогда
		
		ОбработкаЗначенияСвойствОбъекта = Обработки.ЗначенияСвойствОбъекта.Создать();
		
		ОбработкаЗначенияСвойствОбъекта.НазначениеСвойств    = ПланыВидовХарактеристик.НазначенияСвойствКатегорийОбъектов.Справочник_Номенклатура;
		ОбработкаЗначенияСвойствОбъекта.ОбъектОтбораЗначений = Товары;
		
		ТЗСвойствИЗначений = ОбработкаЗначенияСвойствОбъекта.ПрочитатьТаблицуСвойствИЗначений();
		
		Для Каждого СтрокаТЗ Из ТЗСвойствИЗначений Цикл
			
			Если СтрокаТЗ.ПометкаУдаления Или
				 Не СтрокаТЗ.Свойство.ОбязательноеДляТовара Тогда
				Продолжить;
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(СтрокаТЗ.Значение) Тогда
				Сообщить("Для номенклатуры не заполнено обязательное свойство """ + СтрокаТЗ.Наименование + """ !");
				Возврат;
			КонецЕсли;
	...
	Исключение
		...
		Сообщить("Причина: " + ОписаниеОшибки(), СтатусСообщения.Важное);
		Возврат;
	КонецПопытки;
	
	ОбработкиЭтикеток = Обработки.ПечатьЭтикеток.Создать();
	
	ОбработкиЭтикеток.Большая = Большая;
	
	ФормаОбработкиЭтикеток = ОбработкиЭтикеток.ПолучитьФорму();
	...
	Если Режим = "Ценник" Или
		 Режим = "Процентовка" Тогда
		
		ФормаОбработкиЭтикеток.мРежим = Режим;
	...
	Для Каждого Строка из ТаблицаПоТоварам Цикл
		...
		Для Каждого Партия Из Строка.СписанныеПартииБУ Цикл
			...
			Если    СуммаПереоценки   <> 0
				...
				Проводка.Период      = СтруктураШапкиДокумента.Дата;
				Проводка.Организация = СтруктураШапкиДокумента.Организация;
				Проводка.Содержание  = "Списание результата переоценки";
