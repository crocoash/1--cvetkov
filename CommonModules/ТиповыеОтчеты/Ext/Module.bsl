...
функция ОбработкаРасшифровкиТиповогоОтчета(Расшифровка, СтандартнаяОбработка, ОтчетОбъект, ФормаОтчета, ДополнительныеРасшифровки = Неопределено) Экспорт
	...
	ИначеЕсли Команда = "РасшифроватьПоле" Тогда
		...
		СписокДоступныхРасшифровок.Добавить(НСтр("ru='Полный список полей для расшифровки...';uk='Повний список полів для розшифровки...'"));
		Если СписокДоступныхРасшифровок.Количество() > 1 тогда
			...
			КонецЕсли;
		Иначе
			Поле = НСтр("ru='Полный список полей для расшифровки...';uk='Повний список полів для розшифровки...'");
		КонецЕсли;
		
		Если Поле = НСтр("ru='Полный список полей для расшифровки...';uk='Повний список полів для розшифровки...'") тогда
...
Процедура ВывестиТиповойОтчет(ОтчетОбъект, Результат, ДанныеРасшифровки, ВыводВФормуОтчета = Истина, ВнешниеНаборыДанных = Неопределено, ВыводитьШапкуОтчетаНаВсехСтраницах = истина)
	...
	Попытка
		...
		#Если Клиент тогда
			Состояние(НСТР("ru='Если Вы хотите прервать вывод отчета, нажмите Ctrl+Break';uk='Якщо Ви прагнете перервати виведення звіту, натисніть Ctrl+Break'"));
		#КонецЕсли
