...
Процедура ПроверитьНапоминанияПользователя(ВыбПользователь) Экспорт
	...
	ПользователиИГруппы = Новый Массив;
	
	ПользователиИГруппы.Добавить(ВыбПользователь);
	
	БИАС.ПолучитьВсехРодителейЭлемента(ПользователиИГруппы, ВыбПользователь);
	
	// Задачи собираем по всем исполнителям цепочки - по самому пользователю и по
	// всем группам-родителям. Раньше цикл прерывался на первом же исполнителе, у
	// которого нашлись задачи: пока у пользователя висела своя неизменная задача,
	// набор для сверки навсегда застревал на ней, и задачи группы ("Менеджеры"),
	// созданные позже, форму оповещений уже не будили.
	ВыгрузкаДляСверки = Неопределено;
	
	Для Каждого ТекПользователь Из ПользователиИГруппы Цикл
		...
		Запрос.Текст = 
			...
			|			Выполнена = ЛОЖЬ
			|				И Оповещение = ИСТИНА
			|				И СрокОповещения < &ДатаСреза) КАК ЗадачиПользователяЗадачиПоИсполнителю";
		
		
		Запрос.УстановитьПараметр("ТекПользователь", ТекПользователь);
		Запрос.УстановитьПараметр("ДатаСреза"      , ТекущаяДата());
		
		ВыгрузкаИсполнителя = Запрос.Выполнить().Выгрузить();
		
		Если ВыгрузкаДляСверки = Неопределено Тогда
			
			ВыгрузкаДляСверки = ВыгрузкаИсполнителя;
			
		Иначе
			
			Для Каждого СтрокаЗадачи Из ВыгрузкаИсполнителя Цикл
				
				ЗаполнитьЗначенияСвойств(ВыгрузкаДляСверки.Добавить(), СтрокаЗадачи);
	...
	ОткрытьФормуОповещений = ВыгрузкаДляСверки <> Неопределено
		И ВыгрузкаДляСверки.Количество() > 0;
	
	ФормаОповещения = глЗначениеПеременной("ФормаОповещенияОЗадачах");
	
	Если ОткрытьФормуОповещений Тогда
