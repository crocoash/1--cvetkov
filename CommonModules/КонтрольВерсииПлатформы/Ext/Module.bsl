...
Процедура ПроверитьВерсиюПлатформы() Экспорт
	...
	ТекстВопроса = Локализация.СтрШаблон1("Для работы с данной конфигурацией требуется версия платформы 1С:Предприятие ¤1¤ или более поздняя.
		...
		| 
		|Для получения новой версии платформы 1С:Предприятие воспользуйтесь разделом сайта фирмы ""1С"" для пользователей (http://1.v8.1c.ru) или диском Информационно-технологического сопровождения (ИТС)
		|
