...
Процедура ПодготовитьНаборыЗаписейУпр(СтруктураПараметров, ТаблицаСписания, Период, Регистратор, ЗамещатьПриЗаписи)
	...
	Иначе
		СтруктураПараметров.Вставить("ТекНомерСтрокиНезавершенноеПроизводствоУпр", 0);
	КонецЕсли;
...
Процедура ВыполнитьСписание(СтруктураПараметров, ТаблицаСписания, МоментКон,  Останавливаться = Ложь, ПроведениеОстановлено = Ложь, ОтражатьВУправленческомУчете, ОтражатьВБухгалтерскомУчете, Отказ, Партия, Содержание) Экспорт
	...
	Иначе
		...
		Если глЗначениеПеременной("ИспользоватьУправляемыеБлокировки") Тогда
			...
			ОписаниеИсточника.Вставить("Счет", 					"СчетУчетаБУ");
	...
	Если ((КоличествоОсталосьПогасить > 0) И (ОтключитьКонтрольОстатков) И (НЕ СтрокаДокумента.СчетУчетаБУ.Забалансовый)) или 
		...
		Количество = КоличествоОсталосьПогасить;
			...
			Если СтруктураПараметров.ПроводитьДокументПоРазделуУчета Тогда
				...
				Если СчетУчета.НалоговыйУчет Тогда
					ТекстДополнительный = Локализация.СтрШаблон1(НСтр("ru=' , налог. назначение: ¤1¤';uk=' , подат. призначення: ¤1¤'"),СтрокаДокумента.НалоговоеНазначение);
				КонецЕсли;
...
Функция ТорговаяНаценкаНТТНал(ДокументСсылка, Склады, РазделятьПоСтавкамНДС) Экспорт
	
	Запрос = Новый Запрос;
	// В НУ период определения наценки - квартал
	Запрос.УстановитьПараметр("ДатаНач", 			НачалоКвартала(ДокументСсылка.Дата));
	Запрос.УстановитьПараметр("ДатаКон", 			ДокументСсылка.МоментВремени());
	Запрос.УстановитьПараметр("Организация", 		ДокументСсылка.Организация);
	Запрос.УстановитьПараметр("Склады", 			Склады);
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ТоварыВНТТВПродажныхЦенахНалоговыйУчетОстаткиИОбороты.ВидНалоговойДеятельности КАК ВидыНалоговойДеятельности,
	|	ТоварыВНТТВПродажныхЦенахНалоговыйУчетОстаткиИОбороты.СтатьяПриростаУбылиЗапасов КАК СтатьиПриростаУбылиЗапасов,
	|	ТоварыВНТТВПродажныхЦенахНалоговыйУчетОстаткиИОбороты.Склад КАК Склад,"+?(РазделятьПоСтавкамНДС,"
	|	ТоварыВНТТВПродажныхЦенахНалоговыйУчетОстаткиИОбороты.СтавкаНДС КАК СтавкиНДС,","")+"
	|	СУММА(ЕСТЬNULL(ТоварыВНТТВПродажныхЦенахНалоговыйУчетОстаткиИОбороты.СуммаНачальныйОстаток,0) + ЕСТЬNULL(ТоварыВНТТВПродажныхЦенахНалоговыйУчетОстаткиИОбороты.СуммаПриход,0)) КАК ПС,
	|	СУММА(ЕСТЬNULL(ТоварыВНТТВПродажныхЦенахНалоговыйУчетОстаткиИОбороты.СуммаНаценкиНачальныйОстаток,0) + ЕСТЬNULL(ТоварыВНТТВПродажныхЦенахНалоговыйУчетОстаткиИОбороты.СуммаНаценкиПриход,0)) КАК ТН
	|ИЗ
	|	РегистрНакопления.ТоварыВНТТВПродажныхЦенахНалоговыйУчет.ОстаткиИОбороты(&ДатаНач, &ДатаКон, , , Организация = &Организация И Склад В (&Склады)) КАК ТоварыВНТТВПродажныхЦенахНалоговыйУчетОстаткиИОбороты
	|СГРУППИРОВАТЬ ПО
	|	ТоварыВНТТВПродажныхЦенахНалоговыйУчетОстаткиИОбороты.ВидНалоговойДеятельности,
	|	ТоварыВНТТВПродажныхЦенахНалоговыйУчетОстаткиИОбороты.СтатьяПриростаУбылиЗапасов,
	|	ТоварыВНТТВПродажныхЦенахНалоговыйУчетОстаткиИОбороты.Склад"+?(РазделятьПоСтавкамНДС,",
	|	ТоварыВНТТВПродажныхЦенахНалоговыйУчетОстаткиИОбороты.СтавкаНДС","")+"";
