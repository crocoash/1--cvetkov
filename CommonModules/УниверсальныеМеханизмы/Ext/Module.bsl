...
Процедура НапечататьДокумент(ПечДокумент, КоличествоЭкземпляров = 1, НаПринтер = Ложь, Заголовок = "", Ссылка = Неопределено, ИсточникМакета = Неопределено, СтруктураПараметров = Неопределено) Экспорт
	...
	Если ТипЗнч(НаПринтер) = Тип("Булево") Тогда
		...
		Иначе
			...
			ФормаПечати.СтруктураПараметров = СтруктураПараметров;
