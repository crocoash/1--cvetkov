...
Функция ПолучитьПредставлениеАдреса(Запись) Экспорт
	...
	Если СокрЛП(Запись.Поле7) <> "" Тогда
		ТекПредставление = ТекПредставление + ", " + ?(ЗначениеЗаполнено(ТипДома), Локализация.ПолучитьЛокализованныйСинонимОбъекта(ТипДома, Локализация.КодЯзыкаИнформационнойБазы()), НСтр("ru='дом';uk='буд.'",Локализация.КодЯзыкаИнформационнойБазы())) + " № " + СокрЛП(Запись.Поле7);
	КонецЕсли;
...
Функция ПолучитьПредставлениеАдресаПочтовый(Запись) Экспорт
	...
	// Дом
	Если СокрЛП(Запись.Поле7) <> "" Тогда
		ТекПредставление = ТекПредставление + НСтр("ru=', дом № ';uk=', будинок № '",Локализация.КодЯзыкаИнформационнойБазы()) + СокрЛП(Запись.Поле7);
	КонецЕсли;

	// Корпус
	Если СокрЛП(Запись.Поле8) <> "" Тогда
		ТекПредставление = ТекПредставление + НСтр("ru=', корпус ';uk=', корпус '",Локализация.КодЯзыкаИнформационнойБазы()) + СокрЛП(Запись.Поле8);
	КонецЕсли;

	// Квартира
	Если СокрЛП(Запись.Поле9) <> "" Тогда
		ТекПредставление = ТекПредставление + НСтр("ru=', кв.';uk=', кв.'",Локализация.КодЯзыкаИнформационнойБазы()) + СокрЛП(Запись.Поле9);
	КонецЕсли;
