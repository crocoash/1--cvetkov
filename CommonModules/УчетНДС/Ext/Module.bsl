	...
	Для каждого Колонка Из СоответствиеНазваний Цикл
		...
		Если Колонки.Найти(СтароеНазвание) = неопределено тогда
			Если СообщатьОбОшибке Тогда
			    ОбщегоНазначения.СообщитьОбОшибке(Локализация.СтрШаблон1("При переименовании колонок таблиццы не обнаружена колонка с именем ""¤1¤""! Переименование колонки не произведено.", СтароеНазвание));
			КонецЕсли; 
		ИначеЕсли не Колонки.Найти(НовоеНазвание) = неопределено тогда
			Если СообщатьОбОшибке Тогда
			    ОбщегоНазначения.СообщитьОбОшибке(Локализация.СтрШаблон1("При переименовании колонок таблиццы обнаружена колонка с именем ""¤1¤"", переименование существующей колонки ""¤2¤"" невозможно! Переименование колонки не произведено.", НовоеНазвание, СтароеНазвание));
			КонецЕсли; 
		Иначе
