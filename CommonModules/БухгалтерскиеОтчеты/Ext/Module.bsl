	...
	Иначе
		...
		Иначе
			...
			Если ДатаНач <= ДатаКон Тогда
				ОписаниеПериода = "" + ПредставлениеПериода(НачалоДня(ДатаНач), КонецДня(ДатаКон), "Л="+Локализация.ОпределитьКодЯзыкаДляФормат(Локализация.КодЯзыкаИнтерфейса())+"; ФП = Истина");
			Иначе
...
Процедура ОбработчикПриИзмененииСчета(ТабличноеПоле, Знач ТабПолеРазвернутогоСальдо) Экспорт
	...
	Если ЗначениеЗаполнено(Счет) Тогда
		...
		Если НЕ НайденСпособРазворота Тогда
			...
			КонецЕсли;
			Предупреждение(Локализация.СтрШаблон1(НСтр("ru='Счет ""¤1¤"" не имеет ни субсчетов, ни субконто."
...
Процедура ЗаполнитьПараметрыРасшифровкиОборотноСальдовойВедомости(Знач ДатаНач, Знач ДатаКон, Знач ИмяРегистраБухгалтерии,
	...
	ИначеЕсли ИмяГруппировки = "Счет" Тогда
		...
		Если НЕ (Месяц(ДатаНач) = Месяц(ДатаКон) И Год(ДатаНач) = Год(ДатаКон))
			...
			СписокРасшифровки.Добавить(ПараметрыОборотыСчетаМесяц,  Локализация.СтрШаблон1(НСтр("ru='Обороты счета ¤1¤ по месяцам';uk='Обороти рахунку ¤1¤ по місяцях'"), Выборка.СчетПредставление));
			
		КонецЕсли;
		
		СписокРасшифровки.Добавить(ПараметрыОборотыСчетаДень, Локализация.СтрШаблон1(НСтр("ru='Обороты счета ¤1¤ по дням';uk='Обороти рахунку ¤1¤ по днях'"), Выборка.СчетПредставление));
