		...
		Если ЕстьРеквизитПроцентСкидкиНаценки Тогда
			...
			Если ПересчитыватьСкидку Тогда
				...
				Иначе
					...
					Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ЗаказПокупателя") Или
						 ТипЗнч(ДокументОбъект) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
						
						Если СтрокаТабличнойЧасти.Номенклатура.ВидНоменклатуры.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Комплект Тогда
							
							СуммаБезСкидок = СтрокаТабличнойЧасти.КоличествоНК * СтрокаТабличнойЧасти.Цена;
							
						ИначеЕсли ДокументОбъект.ТипЦен.Код = "000000001" Тогда
							
							ЦенаМелкийОпт = Ценообразование.ПолучитьЦенуНоменклатуры(
																					 СтрокаТабличнойЧасти.Номенклатура,
																					 СтрокаТабличнойЧасти.ХарактеристикаНоменклатуры,
																					 Справочники.ТипыЦенНоменклатуры.НайтиПоКоду("000000014"),
																					 ДокументОбъект.Дата,
																					 СтрокаТабличнойЧасти.ЕдиницаИзмерения,
																					 ДокументОбъект.ВалютаДокумента,
																					 ДокументОбъект.КурсВзаиморасчетов,
																					 ДокументОбъект.КратностьВзаиморасчетов,
																					 ,
																					 ДокументОбъект.ДоговорКонтрагента);
							
							СуммаБезСкидок = СтрокаТабличнойЧасти.Количество * ЦенаМелкийОпт;
							
						Иначе
							
							СуммаБезСкидок = СтрокаТабличнойЧасти.Количество * СтрокаТабличнойЧасти.Цена;
...
//  ТаблицаЗначений.
//
Функция ЗапросПоСкидкам(ДатаСкидок,
						МассивНоменклатуры,
						МассивХарактеристик,
						МассивКачество,
						МассивПолучателей,
						ИерархияКонтрагентов,
						КурсКратность,
						СуммаДокумента,
						ВидОплаты,
						ДисконтнаяКарта,
	...
	Запрос.УстановитьПараметр("ИерархияКонтрагентов", ИерархияКонтрагентов);
	...
	|		МАКСИМУМ(ЕСТЬNULL(СкидкиИсключенияКонтрагентовГруппСрезПоследних.КонтрагентГруппа, НЕОПРЕДЕЛЕНО) В (&ИерархияКонтрагентов)
	|				И НЕ СкидкиИсключенияКонтрагентовГруппСрезПоследних.ОтменаИсключения) КАК Исключить
	|	ИЗ
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РегистрСведений.СкидкиИсключенияКонтрагентовГрупп.СрезПоследних(&Дата, ) КАК СкидкиИсключенияКонтрагентовГруппСрезПоследних
	|	ПО
	|		РегСкидкиНоменклатуры.Регистратор = СкидкиИсключенияКонтрагентовГруппСрезПоследних.ДокументУстановкиСкидок
	|	ГДЕ
	...
	|	
	|	ИМЕЮЩИЕ
	|		НЕ МАКСИМУМ(ЕСТЬNULL(СкидкиИсключенияКонтрагентовГруппСрезПоследних.КонтрагентГруппа, НЕОПРЕДЕЛЕНО) В (&ИерархияКонтрагентов)
	|					И НЕ СкидкиИсключенияКонтрагентовГруппСрезПоследних.ОтменаИсключения)
...
Процедура ОбновитьСайт(ТабличнаяЧастьНоменклатуры, Документ) Экспорт  
	...
	КолонкаНоменклатуры = ТабличнаяЧастьНоменклатуры.ВыгрузитьКолонку("Номенклатура");  
		
	Для Каждого Ном Из КолонкаНоменклатуры Цикл 
		
		Если УникальныеНоменклатуры.Найти(Ном) = Неопределено Тогда
			УникальныеНоменклатуры.Добавить(Ном);
		КонецЕсли;
	КонецЦикла;
	
	ОбщегоНазначения.ВыгрузитьТоварыНаСайтВФоне(УникальныеНоменклатуры, Документ);
...
Процедура РассчитатьСкидкиПриПродаже(ДокументОбъект, ТабличнаяЧастьНоменклатуры,
	...
	Для Каждого СтрокаТаблицы Из ТаблицаТоваров Цикл
		...
		Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ЗаказПокупателя") Или
			...
			Если СтрокаТаблицы.Номенклатура.ВидНоменклатуры.ТипНоменклатуры = ТипКомплект Тогда
				
				СтрокаТаблицы.СуммаБезСкидки = СтрокаТаблицы.КоличествоНК * СтрокаТаблицы.Цена;
				
			ИначеЕсли ДокументОбъект.ТипЦен.Код = "000000001" Тогда  //ДокументОбъект.ТипЦен
				...
				СтрокаТаблицы.СуммаБезСкидки = СтрокаТаблицы.Количество * ЦенаБС;
	...
	ИерархияКонтрагентов = Новый Массив;
	
	ИерархияКонтрагентов.Добавить(ДокументОбъект.Контрагент);
	
	//БИАС.ПолучитьВсехРодителейЭлемента(ИерархияКонтрагентов, ДокументОбъект.Контрагент);
	ИерархияКонтрагентов.Добавить(ДокументОбъект.Контрагент.Родитель);
 ...
 //   
 //   //скидку считаем только по номенклатуре для скидок	
 //   Запрос  =  Новый Запрос;
 ...
 //   ТабНоменклатураДЛяСкидок = Запрос.Выполнить().Выгрузить();
 //   
 //   СуммаНоменклатуры = 0;
 //   ДЛя Каждого Стр Из ТаблицаТоваров Цикл
 //   	НайденаНоменклатура = ТабНоменклатураДЛяСкидок.Найти(Стр.Номенклатура,"Номенклатура");
 //   	Если НЕ НайденаНоменклатура = Неопределено Тогда
 //   		СуммаНоменклатуры = СуммаНоменклатуры+Стр.СуммаБезСкидки;
 //   	КонецЕсли;
 //   КонецЦикла;
 ...
 //   		КонецЦикла;
 //   		
 //   		//ОтборПоНоменклатуре = ТаблицаТоваров.НайтиСтроки(Новый Структура("Номенклатура",СтрокаТЧ.Номенклатура));
 //   		//ТабДляИтогаСуммы = ТаблицаТоваров.Скопировать(ОтборПоНоменклатуре);
 //   		//СуммаНоменклатуры = ТабДляИтогаСуммы.Итог("СуммаБезСкидки");
 ...
 //   			Попытка
 //   				Если СтрокаСкидок.НУсловие = Перечисления.УсловияСкидкиНаценки.ПоСуммеДокумента И СуммаНоменклатуры<СтрокаСкидок.НЗначениеУсловия Тогда
 //   					Продолжить;
 //   				КонецЕсли;
 //   			Исключение
 //   				Продолжить;
 //   			КонецПопытки;
 //
 //   			Если СтрокаСкидок.ХарактеристикаНоменклатуры = СтрокаТЧ.ХарактеристикаНоменклатуры Или 
 //   				 СтрокаСкидок.ХарактеристикаНоменклатуры = ПустаяХарактеристика 
 //   				Тогда
 ...
 //   				КонецЕсли;
 //   				//СтрокаСкидок.НЗначениеУсловия
 //   				//СтрокаСкидок.НУсловие
 ...
 //   КонецЦикла;
	...
	//обработаем скидку только для номенклатуры, для которой есть условие
	//Если ПроцентСкидки = Неопределено Тогда 
		Для Каждого Стр Из ТаблицаТоваров Цикл
			...
			//Иначе
				...
				ИнфоПоСкидке = ЦеноОбразование.ПолучитьОбщуюСкидку(Стр.ХарактеристикаНоменклатуры.Владелец, Стр.ХарактеристикаНоменклатуры, ДокументОбъект.ТипЦен, ДокументОбъект.Контрагент, ДокументОбъект.ДоговорКонтрагента,ДокументОбъект.Дата);
				Ск = ИнфоПоСкидке.ПроцентСкидкиНаценки;
				//ДатаОкончанияСкидки = ИнфоПоСкидке.ДатаОкончанияСкидки;

				//Ск = АвтоскидкаБезУсловий(Стр.ХарактеристикаНоменклатуры, ДокументОбъект);
				Если Ск>0 Тогда
					Стр.ПроцентАвтоматическихСкидок = Ск; 
				КонецЕсли;
			//КонецЕсли;
		КонецЦикла;
	//КонецЕсли;
	...
	//Если ДокументОбъект.ТипЦен.Наименование <> "Марина" Тогда
		ПроверкаМинимальнойЦены(ДатаСкидок, ТаблицаТоваров, МинимальныеЦены, СписокФильтров, УчитыватьНДС, СуммаВключаетНДС, ВалютаТовара);
	//КонецЕсли;
...
Функция АвтоскидкаБезУсловий(Характеристика, ДокументОбъект)
	...
	Запрос.Текст = 
		...
		|	УстановкаСкидокНоменклатурыТовары.Ссылка.ДатаНачала КАК Дата,
		|	МАКСИМУМ(УстановкаСкидокНоменклатурыТовары.ПроцентСкидкиНаценки) КАК ПроцентСкидкиНаценки,
		|	МАКСИМУМ(ЕСТЬNULL(СкидкиИсключенияКонтрагентовГруппСрезПоследних.КонтрагентГруппа, НЕОПРЕДЕЛЕНО) В (&ИерархияКонтрагентов)
		|			И НЕ СкидкиИсключенияКонтрагентовГруппСрезПоследних.ОтменаИсключения) КАК Исключить
		|ИЗ
		|	Документ.УстановкаСкидокНоменклатуры.Товары КАК УстановкаСкидокНоменклатурыТовары
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СкидкиИсключенияКонтрагентовГрупп.СрезПоследних(&Дата, ) КАК СкидкиИсключенияКонтрагентовГруппСрезПоследних
		|		ПО УстановкаСкидокНоменклатурыТовары.Ссылка = СкидкиИсключенияКонтрагентовГруппСрезПоследних.ДокументУстановкиСкидок
		|ГДЕ
		|	УстановкаСкидокНоменклатурыТовары.Ссылка.Условие = &Условие
		|	И УстановкаСкидокНоменклатурыТовары.Ссылка.ДатаНачала <= &ДатаНачала
		|	И (УстановкаСкидокНоменклатурыТовары.Ссылка.ДатаОкончания >= &ДатаОкончания
		|			ИЛИ УстановкаСкидокНоменклатурыТовары.Ссылка.ДатаОкончания = &ПустаяДата)
		|	И УстановкаСкидокНоменклатурыТовары.ХарактеристикаНоменклатуры = &Характеристика
		|	И УстановкаСкидокНоменклатурыТовары.Ссылка.Проведен
		|
		|СГРУППИРОВАТЬ ПО
		|	УстановкаСкидокНоменклатурыТовары.Ссылка,
		|	УстановкаСкидокНоменклатурыТовары.Ссылка.ДатаНачала
		|
		|ИМЕЮЩИЕ
		|	НЕ МАКСИМУМ(ЕСТЬNULL(СкидкиИсключенияКонтрагентовГруппСрезПоследних.КонтрагентГруппа, НЕОПРЕДЕЛЕНО) В (&ИерархияКонтрагентов)
		|				И НЕ СкидкиИсключенияКонтрагентовГруппСрезПоследних.ОтменаИсключения)
	...
	Запрос.УстановитьПараметр("Условие"             , Перечисления.УсловияСкидкиНаценки.БезУсловий);
	Запрос.УстановитьПараметр("Дата"                , ДокументОбъект.Дата);
	Запрос.УстановитьПараметр("ДатаНачала"          , НачалоДня(ДокументОбъект.Дата));
	Запрос.УстановитьПараметр("ДатаОкончания"       , КонецДня(ДокументОбъект.Дата));
	Запрос.УстановитьПараметр("ПустаяДата"          , Дата(1, 1, 1));
	Запрос.УстановитьПараметр("Характеристика"      , Характеристика);
	Запрос.УстановитьПараметр("ИерархияКонтрагентов", ИерархияКонтрагентов);
	
	ТЗСкидки = Запрос.Выполнить().Выгрузить();
	
	Если ТЗСкидки.Количество() > 0 Тогда
		
		ПолучателиСкидки = ТЗСкидки[0].Ссылка.ПолучателиСкидки;
		
		Если ПолучателиСкидки.Количество() > 0 Тогда
			
			НайденныеСтроки = ПолучателиСкидки.НайтиСтроки(Новый Структура("Получатель", ДокументОбъект.ДоговорКонтрагента));
			
			Если НайденныеСтроки.Количество() = 0 Тогда
				
				НайденныеСтроки = ПолучателиСкидки.НайтиСтроки(Новый Структура("Контрагент", ДокументОбъект.Контрагент));
		...
		Возврат ТЗСкидки[0].ПроцентСкидкиНаценки;
...
//
Процедура РассчитатьСуммуТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект, СпособРасчета = Неопределено, Реализация = Ложь) Экспорт
	...
	Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ЗаказПокупателя") Или
		...
		Если СтрокаТабличнойЧасти.Номенклатура.ВидНоменклатуры.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Комплект Тогда
			...
			Сумма = СтрокаТабличнойЧасти.КоличествоНК * СтрокаТабличнойЧасти.Цена;
		...
		ИначеЕсли ДокументОбъект.ТипЦен.Код = "000000001" Тогда
			...
			Сумма = СтрокаТабличнойЧасти.Количество * ЦенаБС;
	...
	Если (СпособРасчета = Неопределено)
		...
		Если (СпособРасчета <> Перечисления.СпособРасчетаСуммыДокумента.БезУчетаРучнойСкидки)Тогда
			Если ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("ПроцентСкидкиНаценки", МетаданныеДокумента, ИмяТабличнойЧасти) Тогда
				СуммаСкидки = СуммаСкидки + Окр((Сумма * СтрокаТабличнойЧасти.ПроцентСкидкиНаценки / 100),0,1);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	//Если ДокументОбъект.ТипЦен.Наименование = "Марина" И СтрокаТабличнойЧасти.ПроцентАвтоматическихСкидок <> 0 Тогда
	//	Если Не Реализация Тогда
	//		НоваяЦена = СтрокаТабличнойЧасти.Цена + СтрокаТабличнойЧасти.Цена * (СтрокаТабличнойЧасти.ПроцентАвтоматическихСкидок /100);
	//		СтараяЦена = СтрокаТабличнойЧасти.Цена; 
	//		СтрокаТабличнойЧасти.Сумма = СтараяЦена;
