...
//
Процедура ПереоценкаСчетаРегл(СтруктураШапкиДокумента,ПроводкиБУ,СчетПереоценки, ВалютаРегламентированногоУчета="",ВставитьПервой=Истина,РассчитыватьОстатки=Истина, Заголовок = Неопределено, ЧастичнаяПереоценка=Ложь, ЗаписьРасчетаПереоценкиВалюты = Неопределено) экспорт

	ЕстьНалогНаПрибыль = Ложь;
	Если НЕ СтруктураШапкиДокумента.Свойство("ЕстьНалогНаПрибыль", ЕстьНалогНаПрибыль) Тогда
		ЕстьНалогНаПрибыль = Ложь;
	КонецЕсли;
	...
	Если РассчитыватьОстатки тогда
		...
		Для каждого Стр из РеестрОстатков цикл
			...
			Проводка.Содержание  = "Переоценка счета";
			Если КурсоваяРазница<0 тогда
...
Функция ПереоценкаСчетовДокументаРегл(ДокументОбъект, СтруктураШапкиДокумента, ВалютаРегламентированногоУчета="",Отказ) экспорт
	...
	СтруктураШапкиДокумента.Вставить("СчетаУчетаКР", ПараметрыУчетаКурсовыхРазниц(УчетнаяПолитика.ИспользуемыеКлассыСчетовРасходов));
	...
	Для каждого ТекСчет из ТаблицаСчетов цикл
		...
		ПереоценкаСчетаРегл(СтруктураШапкиДокумента, ПроводкиБУ, СчетДляПереоценки, ВалютаРегламентированногоУчета, Истина);
...
Процедура ПроверитьЗаполненостьГруппыРеквизитовСхемыРеализации(ПрефиксГруппыТЗ, ПрефиксГруппы,
	...
	Для Н = 1 По КвоСубконто Цикл
		...
		Если НЕ ЗначениеЗаполнено(ПроверяемыйРеквизитТЗ) Тогда
			...
			ОбщегоНазначения.СообщитьОбОшибке(Символы.Таб + Локализация.СтрШаблон1("Не заполнено значение субконто ""¤1¤"" счета ""¤2¤"".", ВидыСубконто[Н - 1].ВидСубконто.Наименование, ПредставлениеСчета), Отказ);
...
Процедура ПроверитьСхемыРеализацииТабличнойЧастиНаЗаполненость(ДокументОбъект, ИмяТабличнойЧасти,
	...
	Пока ТЗ.Количество() > 0 Цикл
		...
		СтрокаНачалаСообщенияОбОшибке = Локализация.СтрШаблон1("Бухг. учет: Табличная часть ""¤1¤"". Строка номер ""¤2¤"". ¤3¤ ""¤4¤"".", ПредставлениеТабличнойЧасти, НомераСтрок, ПредставлениеСхемыРеализации, СхемаРеализации);
		...
		Если ТипЗнч(СписокПараметров) = Тип("Структура") Тогда
			Если СписокПараметров.Свойство("Контрагент") Тогда
				Если ТипЗнч(СписокПараметров.Контрагент) =  Тип("СправочникСсылка.Контрагенты") Тогда
					...
					ФормаВыбора.СправочникСписок.Отбор.Владелец.ВидСравнения  = ВидСравнения.Больше;
					ФормаВыбора.ЭлементыФормы.СправочникСписок.НастройкаОтбора.Владелец.Доступность = Ложь;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли; 
