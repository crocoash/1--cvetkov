...
Попытка
			Если ЗначениеЗаполнено(СтрокаТаблицы.Номенклатура)
			   И  СтрокаТаблицы.Услуга Тогда
				...
				КонецЕсли;
			Исключение
				Сообщить(СокрЛП(ДокументОбъект));
			КонецПопытки;
...
Процедура ПроверитьЧтоСкладНеНТТ(ДокументОбъект, ИмяТабличнойЧасти, ТаблицаЗначений, 
	...
	Для каждого СтрокаТаблицы Из ТаблицаЗначений Цикл
		...
		Если СтрокаТаблицы[ИмяВидаСкладаРазмещения] = Перечисления.ВидыСкладов.НТТ Тогда
				...
				ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "содержится размещение по неавтоматизированной торговой точке. " +
		...
		Для каждого СтрокаТаблицы Из ТаблицаЗначений Цикл
			...
			Если ЗначениеЗаполнено(СтрокаТаблицы.Номенклатура)
			   И  СтрокаТаблицы.БланкСтрогогоУчета = Истина Тогда
					...
					ОбщегоНазначения.ОшибкаПриПроведении(Локализация.СтрШаблон1("В строке номер ""¤1¤"" табличной части ""¤2¤"": содержится бланк строгого учета. Бланков строгого учета здесь быть не должно!", СокрЛП(СтрокаТаблицы.НомерСтроки), ПредставлениеТабличнойЧасти), Отказ, Заголовок);
		...
		Для каждого СтрокаТаблицы Из ДокументОбъект[ИмяТабличнойЧасти] Цикл
			...
			Если ЗначениеЗаполнено(СтрокаТаблицы.Номенклатура)
			   И  СтрокаТаблицы.Номенклатура.БланкСтрогогоУчета Тогда
					...
					ОбщегоНазначения.ОшибкаПриПроведении(СтрокаНачалаСообщенияОбОшибке + " содержится бланк строгого учета. Бланков строгого учета здесь быть не должно", Отказ, Заголовок);
		...
		Для каждого СтрокаТаблицы Из ТаблицаЗначений Цикл
			...
			Если ЗначениеЗаполнено(СтрокаТаблицы.Номенклатура)
			   И  Не СтрокаТаблицы.БланкСтрогогоУчета Тогда
					...
					ОбщегоНазначения.ОшибкаПриПроведении(СтрокаНачалаСообщенияОбОшибке + "содержится номенклатура, не являющаяся бланком строгого учета. Здесь могут быть только бланки строгого учета!", 
										Отказ, Заголовок);
...
Процедура ДобавитьСоставНабора(СтрокаТабличнойЧасти, ДокументОбъект) Экспорт
	...
	Иначе
		...
		БИАС.ПолучитьКомплектующиеНаборКомплекта(СтрокаТабличнойЧасти, ТаблицаКомплектующих);
		
		ТаблицаКомплектующих.Сортировать("Номенклатура, ХарактеристикаНоменклатуры");
		
		ТаблицаКомплектующих.Колонки.Добавить("КлючСтроки");
		
		ТаблицаКомплектующих.ЗаполнитьЗначения(СтрокаТабличнойЧасти.КлючСтроки, "КлючСтроки");
...
Функция СформироватьТаблицуКомплектующих(ТаблицаИсточник, ДокументОбъект, СтруктураШапкиДокумента = Неопределено) Экспорт
	...
	Если НайденныеКомплекты.Количество() > 0 Тогда
		...
		ЕстьСтатьяПриростаУбылиЗапасов  = ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("СтатьяПриростаУбылиЗапасов", МетаданныеДокумента, "СоставНабора");
		...
		|" + ?(ЕстьСтатьяПриростаУбылиЗапасов, "Док.СтатьяПриростаУбылиЗапасов КАК СтатьяПриростаУбылиЗапасов,","") + "
		...
		Для Каждого СтрокаКомплект Из НайденныеКомплекты Цикл
			...
			Для Каждого СтрокаКомплектующая Из НайденныеКомплектующие Цикл
				...
				Для Сч = 1 По ТаблицаРезультат.Колонки.Количество() Цикл
					...
					ИначеЕсли ЕстьСтатьяПриростаУбылиЗапасов И ИмяКолонки = "СтатьяПриростаУбылиЗапасов" Тогда
						НоваяСтрока.Установить(Индекс, СтрокаКомплектующая.СтатьяПриростаУбылиЗапасов);
