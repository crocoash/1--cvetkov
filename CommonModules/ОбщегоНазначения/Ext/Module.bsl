...
Функция ОписаниеДляХарактеристики(Характеристика, Рус, ПринудительноДобавлятьЗамеры = Ложь, ДобавлятьЗамеры = Истина) Экспорт 
	...
	// ДобавлятьЗамеры = Ложь передаёт выгрузка нашего сайта: там таблица замеров
	// уходит отдельным тегом <measurements>, и приклеенная к описанию копия
	// показывалась покупателю второй раз. Маркетплейсам (Розетка, Каста, Пром,
	// Эпицентр, Алло, KidsStaff) отдельного поля под замеры не положено, им
	// таблица по-прежнему нужна внутри описания — поведение по умолчанию прежнее.
	Если ДобавлятьЗамеры И (НетФотоЗамеров Или ПринудительноДобавлятьЗамеры) Тогда
...
// Розничная цена позиций в гривне так, как её показывает карточка товара на сайте.
//
// Скидку витрины (уточняющий процент наценки, скидки характеристик) 1С считает
// только в выгрузке на сайт - ценообразование и автоматические скидки документа её
// не знают: «Перезаполнить цены» в заказе 28270 давало прайс 3100 при цене сайта
// 2520. Здесь те же данные, что уходят на сайт в <priceRoz> и <discount_rozn>
// (ПолучитьТаблицуОстатков с настройками ВыгрузкаНашСайт), и то же округление, что
// делает сайт (shop.class.php, getPriceA/getPriceC): скидка там хранится с двумя
// знаками, цена со скидкой округляется вверх до гривны, затем вверх до кратного 5.
// Сверено с витриной 15.09.2026: 3100 -> 2520, 2990 -> 2435, 2900 -> 2350.
//
// Параметры:
//  МассивНоменклатуры - Массив - номенклатура строк документа, повторы допустимы
//
// Возвращаемое значение:
//  Соответствие - ключ СправочникСсылка.ХарактеристикиНоменклатуры, значение Число;
//                 позиций, которых нет в выгрузке сайта, в нём нет
//
Функция ЦеныВитриныСайта(МассивНоменклатуры) Экспорт
	...
	Для Каждого Номенклатура Из МассивНоменклатуры Цикл
		...
		Если Не ЗначениеЗаполнено(Номенклатура) Или Обработанные[Номенклатура] <> Неопределено Тогда
			Продолжить;
		КонецЕсли;

		Обработанные.Вставить(Номенклатура, Истина);

		Выборка = ПолучитьТаблицуОстатков(Номенклатура, Null, Истина, , ВариантВыгрузки, Истина);

		Пока Выборка.Следующий() Цикл

			ЦенаСайта = ЦенаВитриныСайта(Выборка.ЦенаРозница, Выборка.СкидкаРозница);

			Если ЦенаСайта > 0 Тогда
				Цены.Вставить(Выборка.Характеристика, ЦенаСайта);
			КонецЕсли;
...
// Цена карточки товара на сайте в гривне по розничной цене и проценту скидки
// выгрузки - повторяет getPriceA/getPriceC сайта (курс гривны 1).
//
Функция ЦенаВитриныСайта(ЦенаРозница, СкидкаРозница)

	Цена = ?(ТипЗнч(ЦенаРозница) = Тип("Число"), ЦенаРозница, 0);

	Если Цена <= 0 Тогда
		Возврат 0;
	КонецЕсли;

	Скидка = ?(ТипЗнч(СкидкаРозница) = Тип("Число"), Окр(СкидкаРозница, 2), 0);

	Если Скидка <> 0 Тогда
		Цена = ОкруглитьВверхДоКратного(Цена * (100 - Скидка) / 100, 1);
	КонецЕсли;

	Возврат ОкруглитьВверхДоКратного(Цена, 5);

КонецФункции

// Округление положительного значения вверх до кратного (PHP ceil).
//
Функция ОкруглитьВверхДоКратного(Значение, Кратность)

	Частное = Значение / Кратность;
	Целое   = Цел(Частное);

	Возврат ?(Частное > Целое, Целое + 1, Целое) * Кратность;
