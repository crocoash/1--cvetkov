...
Функция ПолучитьСуммуСкидки(Сумма, Скидка) Экспорт
	...
	Если ЗначениеЗаполнено(Скидка) И Скидка <> 100 Тогда
		СуммаСкидки = Окр(Сумма / (100 - Скидка) * Скидка,0,1);
	Иначе
		СуммаСкидки = 0;
	КонецЕсли;
...
Функция ПолучитьЦенуНоменклатуры(Номенклатура, ХарактеристикаНоменклатуры = Неопределено, Знач ТипЦен, ДатаПараметр = Неопределено, ЕдиницаИзмерения = Неопределено,
                                 ...
                                 УсловиеПродаж = Неопределено, ИспользоватьПервыеЦены = Ложь, Наценка = Неопределено) Экспорт
	...
	Если ПолученнаяЦена = 0 И ЗначениеЗаполнено(ТипЦен) Тогда // особых условий нет
		...
		ПолученнаяЦена = ПолучитьЦенуДляВыгрузки(Дата,Номенклатура,ХарактеристикиНоменклатуры,ХарактеристикаНоменклатуры, ложь, ТипЦен, Валюта, Курс, ИспользоватьПервыеЦены, Наценка, ЕдиницаИзмерения, Кратность);
		
		Если ПолученнаяЦена = 0 ИЛи ПолученнаяЦена = Неопределено Тогда
			
			ПолученнаяЦена = ПолучитьЦенуДляВыгрузки(Дата,Номенклатура,ХарактеристикиНоменклатуры,ХарактеристикаНоменклатуры, Истина,ТипЦен, Валюта, Курс, ИспользоватьПервыеЦены, Наценка, ЕдиницаИзмерения, Кратность); 
	...
	Возврат ?(ПолученнаяЦена = Неопределено,0,ПолученнаяЦена); 

КонецФункции // ПолучитьЦенуНоменклатуры()


функция ПолучитьЦенуДляВыгрузки(Дата,Номенклатура,ХарактеристикиНоменклатуры, ХарактеристикаНоменклатуры, Рассчитывается, ТипЦен, Валюта, Курс, ИспользоватьПервыеЦены, Наценка, ЕдиницаИзмерения, Кратность)
		...
		Если Рассчитывается Тогда // надо достать цену базового типа и рассчитать по наценке
			
			УточняющийПроцент = ПолучитьУточняющийПроцент(Дата, ТипЦен, Номенклатура, ХарактеристикаНоменклатуры);
						
			Если ИспользоватьПервыеЦены Тогда 
				Если Наценка <> Неопределено Тогда 
					ПроцентНаценки = Наценка;
				Иначе
					ПроцентНаценки = ТипЦен.ПроцентСкидкиНаценки;	
				КонецЕсли;
				
			Иначе
				ПроцентНаценки = ТипЦен.ПроцентСкидкиНаценки * УточняющийПроцент / 100;
			КонецЕсли;						
		...
		Если ИспользоватьПервыеЦены Тогда
            Запрос.Текст = СтрЗаменить(Запрос.Текст, "ЦеныНоменклатурыСрезПоследних", "ЦеныНоменклатурыСрезПервых");
        КонецЕсли;
		...
		Если Выборка.Следующий() Тогда
			Если Рассчитывается Тогда
				...
				ИначеЕсли Выборка.СпособРасчетаЦены = Перечисления.СпособыРасчетаЦены.НаСумму Тогда
					ПолученнаяЦена = ПолученнаяЦена + ТипЦен.СуммаСкидкиНаценки;
				КонецЕсли;
			Иначе
				ВалютаТекущая = Выборка.Валюта;

				//Если ТипЦен.БазовыйтипЦен <> Справочники.ТипыЦенНоменклатуры.ПустаяСсылка() Тогда
				//	ЦенаСоСкидкой = Выборка.Цена;
				//	
				//	Контрагент = Справочники.Контрагенты.НайтиПоКоду("000000258");
				//	Договор = Справочники.ДоговорыКонтрагентов.НайтиПоКоду("000000281");

				//	ИнфоПоСкидке = ПолучитьОбщуюСкидку(Номенклатура, ХарактеристикаНоменклатуры, Контрагент, Договор);
				//	Скидка = ИнфоПоСкидке.ПроцентСкидкиНаценки;
				//    ПолученнаяЦена = ЦенаСоСкидкой * (1 - Скидка / 100);
				//Иначе	
...
Функция СформироватьЗапросПоЦенам(СтруктураЗначений, СпособЗаполненияЦен, МассивНоменклатуры,
	...
	Если СпособЗаполненияЦен = Перечисления.СпособыЗаполненияЦен.ПоЦенамНоменклатуры Тогда
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |	БазовыеЦены.Номенклатура.ЕдиницаХраненияОстатков КАК НоменклатураЕдиницаХраненияОстатков,
		               ...
		               |		ГДЕ
		               |			СпрНоменклатура.Ссылка В(&МассивНоменклатуры)) КАК СпрНоменклатура
		               ...
		               |			ПО (ТипыЦенПоГруппам.НоменклатурнаяЦеноваяГруппа = СпрНоменклатура.Номенклатура.ЦеноваяГруппа
		               |					ИЛИ ТипыЦенПоГруппам.НоменклатурнаяЦеноваяГруппа = СпрНоменклатура.Номенклатура.НоменклатурнаяГруппа)
		               |			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры.СрезПоследних(&Дата, Номенклатура В (&МассивНоменклатуры)) КАК ЦеныНоменклатуры
		               |			ПО (ЦеныНоменклатуры.Номенклатура = СпрНоменклатура.Номенклатура)
		               |				И (ЦеныНоменклатуры.ТипЦен = ТипыЦенПоГруппам.ТипЦен
		               |					ИЛИ (ТипыЦенПоГруппам.ТипЦен ЕСТЬ NULL
		               |						ИЛИ ТипыЦенПоГруппам.ТипЦен = ЗНАЧЕНИЕ(Справочник.ТипыЦенНоменклатуры.ПустаяСсылка))
		               |						И ЦеныНоменклатуры.ТипЦен = &ТипЦен)
		               |			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.УсловияПоставокПоДоговорамКонтрагентовПоНоменклатуре.СрезПоследних(
		               |					&Дата,
		               |					ДоговорКонтрагента = &ДоговорКонтрагента
		               |						И Номенклатура В (&МассивНоменклатуры)) КАК УсловияПоставок
		               |			ПО (УсловияПоставок.Номенклатура = ЦеныНоменклатуры.Номенклатура
		               ...
		               |	ГДЕ
		               |		СпрНоменклатура.Номенклатура В(&МассивНоменклатуры)) КАК БазовыеЦены
		               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры.СрезПоследних(&Дата, Номенклатура В (&МассивНоменклатуры)) КАК ЦеныНоменклатурыРассчитываемые
		               |		ПО БазовыеЦены.Номенклатура = ЦеныНоменклатурыРассчитываемые.Номенклатура
		               ...
		               |		ПО (БазовыеЦены.Номенклатура.ЦеноваяГруппа = НаценкиПоУсловиямПродаж.НоменклатурнаяЦеноваяГруппа
		...
		Иначе // присваиваем значение цены и единицы цены
			...
			Если ЭтоЦеныНоменклатуры И СтруктураЗначений.НовыйТипЦен.Рассчитывается Тогда
				...
				Если ВыборкаИзРезультатаЗапроса.СпособРасчетаЦены = Перечисления.СпособыРасчетаЦены.ПоПроцентнойНаценкеНаБазовыйТип Тогда
					...
					УточняющийПроцент = ПолучитьУточняющийПроцент(ДокументОбъект.Дата, ВыборкаИзРезультатаЗапроса.ТипЦен , ХарактеристикаНоменклатуры.Владелец, ХарактеристикаНоменклатуры);
					
					ПроцентСкидкиНаценки = ?(ВыборкаИзРезультатаЗапроса.ПроцентСкидкиНаценки,ВыборкаИзРезультатаЗапроса.ПроцентСкидкиНаценки,0) * ?(УточняющийПроцент,УточняющийПроцент,0) / 100;
					
					Цена = Цена * (1 + ПроцентСкидкиНаценки / 100);
					
					Цена = МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(
																		   Цена,
				...
				ИначеЕсли ВыборкаИзРезультатаЗапроса.СпособРасчетаЦены = Перечисления.СпособыРасчетаЦены.НаСумму Тогда
					
					Цена = Цена + СтруктураЗначений.НовыйТипЦен.СуммаСкидкиНаценки;
...
Функция ПолучитьУточняющийПроцент(Дата, ТипЦен, Номенклатура, Характеристика = Неопределено) Экспорт	
	
	Если ТипЦен.НеПрименятьУточняющийПроцент Тогда
		Возврат 100;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Характеристика) Тогда 
		Цвет = Характеристика.ПолучитьОбъект().ПолучитьЗначениеЦвета();
	КонецЕсли;
	...
	Запрос.Текст = 
		...
		"ВЫБРАТЬ
		|	ВЫБОР
		|		КОГДА ЕСТЬNULL(ПолучимПроцентРодителя.Период, ДАТАВРЕМЯ(1, 1, 1)) >= ЕСТЬNULL(ПолучимПроцентНоменклатурыПоЦвету.Период, ДАТАВРЕМЯ(1, 1, 1))
		|				И ЕСТЬNULL(ПолучимПроцентРодителя.Период, ДАТАВРЕМЯ(1, 1, 1)) >= ЕСТЬNULL(ПолучимПроцентНоменклатурыБезЦвета.Период, ДАТАВРЕМЯ(1, 1, 1))
		|			ТОГДА ПолучимПроцентРодителя.УточняющийПроцент
		...
		|			НайдемПроцентРодителя.УточняющийПроцент КАК УточняющийПроцент,
		|			НайдемПроцентРодителя.Период КАК Период,
		|			НайдемПроцентРодителя.НоменклатураИлиГруппа КАК Номенклатура1
		|		ИЗ
		|			РегистрСведений.УточнениеРасчетаЦенПоНоменклатуре.СрезПоследних(
		|					&ТекущаяДата,
		...
		|						И Период <= &ТекущаяДата) КАК НайдемПроцентРодителя) КАК ПолучимПроцентРодителя
		|		ПО Номенклатура.Родитель = ПолучимПроцентРодителя.Номенклатура1
		|		ПОЛНОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
		|			ЕСТЬNULL(ПолучимПроцентНоменклатурыПоЦвету.Период, ДАТАВРЕМЯ(1, 1, 1)) КАК Период,
		|			ЕСТЬNULL(ПолучимПроцентНоменклатурыПоЦвету.УточняющийПроцент, 11) КАК УточняющийПроцент,
		|			ПолучимПроцентНоменклатурыПоЦвету.Цвет КАК Цвет,
		|			ПолучимПроцентНоменклатурыПоЦвету.НоменклатураИлиГруппа КАК Номенклатура1
		|		ИЗ
		|			РегистрСведений.УточнениеРасчетаЦенПоНоменклатуре.СрезПоследних(
		|					&ТекущаяДата,
		|					НоменклатураИлиГруппа = &Номенклатура
		|						И Цвет = &Цвет
		|						И Период <= &ТекущаяДата) КАК ПолучимПроцентНоменклатурыПоЦвету) КАК ПолучимПроцентНоменклатурыПоЦвету
		|		ПО Номенклатура.Ссылка = ПолучимПроцентНоменклатурыПоЦвету.Номенклатура1
		|		ПОЛНОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
		|			ПолучимПроцентНоменклатурыБезЦвета.Период КАК Период,
		|			ПолучимПроцентНоменклатурыБезЦвета.УточняющийПроцент КАК УточняющийПроцент,
		|			ПолучимПроцентНоменклатурыБезЦвета.НоменклатураИлиГруппа КАК Номенклатура1
		|		ИЗ
		|			РегистрСведений.УточнениеРасчетаЦенПоНоменклатуре.СрезПоследних(
		|					&ТекущаяДата,
		|					НоменклатураИлиГруппа = &Номенклатура
		|						И Цвет = ЗНАЧЕНИЕ(Справочник.АСБ_ЦветаНоменклатуры.ПустаяСсылка)
		|						И Период <= &ТекущаяДата) КАК ПолучимПроцентНоменклатурыБезЦвета) КАК ПолучимПроцентНоменклатурыБезЦвета
		|		ПО Номенклатура.Ссылка = ПолучимПроцентНоменклатурыБезЦвета.Номенклатура1
		|ГДЕ
		|	Номенклатура.Ссылка = &Номенклатура";
	...
	Если Выборка.Следующий() Тогда
		Если ЗначениеЗАполнено(Выборка.наценка) Тогда
...
Процедура ПолучитьВсехРодителей(Элемент, МассивРодителей) Экспорт
	...
	ПолучитьВсехРодителей(Родитель, МассивРодителей);
	
КонецПроцедуры


функция ПолучитьОбщуюСкидкуДляТипаПользователя(Номенклатура, Характеристика,ВидКонтрагента) Экспорт 

	Если Перечисления.ТипКонтрагента.Дроп = ВидКонтрагента Тогда
		Контрагент = Неопределено;
	ИначеЕсли Перечисления.ТипКонтрагента.Дроп = ВидКонтрагента Тогда
		Контрагент = Справочники.Контрагенты.НайтиПоКоду("000000725");
	ИначеЕсли Перечисления.ТипКонтрагента.Опт = ВидКонтрагента Тогда
		Контрагент = Справочники.Контрагенты.НайтиПоКоду("000000884");
	ИначеЕсли Перечисления.ТипКонтрагента.Сп = ВидКонтрагента Тогда
		Контрагент = Справочники.Контрагенты.НайтиПоКоду("000007729");
	КонецЕсли;
	
	Возврат ПолучитьОбщуюСкидку(Номенклатура, Характеристика, Контрагент);
	
КонецФункции

Функция ПолучитьОбщуюСкидку(Номенклатура, Характеристика, ТипЦен = Неопределено, Контрагент = Неопределено, Договор = Неопределено, ДатаНачала = Неопределено) Экспорт
	
	РезультатПоДоговору = Новый Структура;
    РезультатПоДоговору.Вставить("ПроцентСкидкиНаценки", 0);
	РезультатПоДоговору.Вставить("ДатаОкончанияСкидки", Неопределено);
	РезультатПоДоговору.Вставить("ДатаНачала", Неопределено);
	
	
	Хар = Характеристика;
	Если ЗначениеЗаполнено(ТипЦен) Тогда
		Если НЕ Характеристика.ПолучитьОбъект().ЭтоХарактеристикаНК() И (ТипЦен.Код = "000000001" ИЛИ ТипЦен.Код = "000000014" ИЛИ  ТипЦен.Код = "000000002")  Тогда
			Хар = Характеристика.ПолучитьОбъект().ХарактеристикаРостовки(); 
			Если не ЗначениеЗаполнено(Хар) Тогда Возврат РезультатПоДоговору; КонецЕсли; 
		КонецЕсли;
	...
	ДатаНачало = ?(ДатаНачала=Неопределено,НачалоДня(ТекущаяДата()),ДатаНачала);
	//ДатаНачало = формат(ДатаНачало,"ДФ=dd.MM.yyyy");	
	
	// Базовый текст запроса для скидки по договору
	Запрос.Текст = 
	    "ВЫБРАТЬ
		|	СкидкиНаценкиНоменклатурыСрезПоследних.ПроцентСкидкиНаценки КАК ПроцентСкидкиНаценки,
	    |	СкидкиНаценкиНоменклатурыСрезПоследних.ДатаОкончания КАК ДатаОкончанияСкидки,
	    |	СкидкиНаценкиНоменклатурыСрезПоследних.Регистратор КАК Регистратор, 
		   |	СкидкиНаценкиНоменклатурыСрезПоследних.Регистратор.ДатаНачала КАК ДатаНачала

	    |ИЗ
	    |	РегистрСведений.СкидкиНаценкиНоменклатуры КАК СкидкиНаценкиНоменклатурыСрезПоследних
	    |ГДЕ
	    |	СкидкиНаценкиНоменклатурыСрезПоследних.Номенклатура = &Номенклатура
	    |	И СкидкиНаценкиНоменклатурыСрезПоследних.ХарактеристикаНоменклатуры = &Характеристика
	    |	И (КОНЕЦПЕРИОДА(СкидкиНаценкиНоменклатурыСрезПоследних.ДатаОкончания, ДЕНЬ) >= &Дата
	    |			ИЛИ СкидкиНаценкиНоменклатурыСрезПоследних.ДатаОкончания = ДАТАВРЕМЯ(1, 1, 1))
	    |	И КОНЕЦПЕРИОДА(СкидкиНаценкиНоменклатурыСрезПоследних.Регистратор.ДатаНачала, ДЕНЬ) <= &Дата
	    |	И (СкидкиНаценкиНоменклатурыСрезПоследних.Регистратор.ПолучателиСкидки.Получатель = &Договор
	    |			ИЛИ СкидкиНаценкиНоменклатурыСрезПоследних.Регистратор.ДляВсехПолучателей
	    |				И НЕ СкидкиНаценкиНоменклатурыСрезПоследних.Регистратор.ИсключитьКонтрагентаГруппу.КонтрагентГруппа = &Контрагент";
		
			 
		Запрос.УстановитьПараметр("Договор", Договор);
		Запрос.УстановитьПараметр("Дата", ДатаНачало);
		Запрос.УстановитьПараметр("Номенклатура", Хар.Владелец);
		...
		Если Контрагент <> Неопределено И контрагент <> Справочники.Контрагенты.ПустаяСсылка() Тогда 
			Запрос.Текст = Запрос.Текст + "
			    |				И НЕ СкидкиНаценкиНоменклатурыСрезПоследних.Регистратор.ИсключитьКонтрагентаГруппу.КонтрагентГруппа = &Родитель
			    |				И НЕ СкидкиНаценкиНоменклатурыСрезПоследних.Регистратор.ИсключитьКонтрагентаГруппу.КонтрагентГруппа = &РодительРодитель)
				| УПОРЯДОЧИТЬ ПО
	    		|	ДатаНачала УБЫВ";		
			Запрос.УстановитьПараметр("Родитель", Контрагент.Родитель);
			Запрос.УстановитьПараметр("РодительРодитель",Контрагент.Родитель.Родитель);
		Иначе
			Запрос.Текст = Запрос.Текст + ")
			 |  УПОРЯДОЧИТЬ ПО
	   		 |	ДатаНачала УБЫВ ";
	...
	// Заполняем результат по договору, если он существует
	Если Рез.Следующий() Тогда
		Возврат Рез;	
	Иначе
		Возврат РезультатПоДоговору;
