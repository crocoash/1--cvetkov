...
Процедура ПоказатьСтруктуруПодчиненностиДокумента(ДокументСсылка) Экспорт
	...
	СледВложение = ДокументСсылка;
	Док = ДокументСсылка; 	
	Пока Док <> Неопределено Цикл
		Если СледВложение.Метаданные().Реквизиты.Найти("ОсновнойЗаказ") <> Неопределено Тогда	
			Если СледВложение.ОсновнойЗаказ <> Неопределено Тогда
				СледВложение = СледВложение.ОсновнойЗаказ;
			Иначе
				Док = Неопределено;
			КонецЕсли;		
		ИначеЕсли СледВложение.Метаданные().Реквизиты.Найти("Сделка") <> Неопределено  Тогда 
			Если СледВложение.Сделка <> Неопределено Тогда
				СледВложение = СледВложение.Сделка;
			Иначе
				Док = Неопределено;
			КонецЕсли;	
		ИначеЕсли СледВложение.Метаданные().Реквизиты.Найти("ДокументОснование") <> Неопределено  Тогда 
			Если СледВложение.ДокументОснование <> Неопределено Тогда
				СледВложение = СледВложение.ДокументОснование;
			Иначе
				Док = Неопределено;
			КонецЕсли;	
		Иначе 
	...
	ФормаСтруктурыПодчиненности.ДокументСсылка = СледВложение;
...
// Выполняет общие действия по установке реквизитов налогового учета
// при изменении НалоговоеНазначение
//
// Параметры
//	НалоговоеНазначение        - налоговое назначение
//  СтатьяПриростаУбылиЗапасов - статья прироста/убыли запасов
// 	УчитываетсяКакЗапас        – признак "учитывается как запас"
//	Номенклатура                - номенклатура, для которой определяются параметры
//                               (используется для определения значений по умолчанию)
//	Организация                - организация, для которой определяются параметры
//                               (используется для определения значений по умолчанию)
//	Склад                      - склад, для которого определяются параметры
//                               (используется для определения значений по умолчанию)
//
Процедура ПриИзмененииНалоговоеНазначениеЗапасов(НалоговоеНазначение, СтатьяПриростаУбылиЗапасов, 
	                                             Номенклатура = Неопределено,
										         Организация = Неопределено, 
	                                             Склад = Неопределено,
										         УчитываетсяКакЗапас = Истина) Экспорт
	...
	СтатьяОборудование = Справочники.СтатьиНалоговыхДеклараций.НП_К1_1_Оборудование;
	СчетаУчета         = Неопределено;
    Амортизируется     = НалоговоеНазначение.Амортизируется;
	
	УчитываетсяКакЗапас = НЕ Амортизируется;
	
	Если Амортизируется Тогда
		
		СтатьяПриростаУбылиЗапасов = СтатьяОборудование;
		
	Иначе
		
		Если СтатьяПриростаУбылиЗапасов = СтатьяОборудование Тогда
			
			СчетаУчета                 = БухгалтерскийУчет.ПолучитьСчетаУчетаНоменклатуры(Организация, Номенклатура, Склад);
			СтатьяПриростаУбылиЗапасов = СчетаУчета.СтатьяПриростаУбылиЗапасов;
			
			Если СтатьяПриростаУбылиЗапасов = СтатьяОборудование Тогда
				
				СтатьяПриростаУбылиЗапасов = Неопределено;
...
КонецПроцедуры // ПриИзмененииНалоговоеНазначениеЗапасов()

// Выполняет общие действия по установке реквизитов налогового учета
// при изменении УчитываетсяКакЗапас
//
// Параметры
// 	УчитываетсяКакЗапас        – признак "учитывается как запас"
//  СтатьяПриростаУбылиЗапасов - статья прироста/убыли запасов
...
//                               (используется для определения значений по умолчанию)
//
Процедура ПриИзмененииУчитываетсяКакЗапас(УчитываетсяКакЗапас, СтатьяПриростаУбылиЗапасов,
	                                      НалоговоеНазначение,
	                                      Номенклатура = Неопределено,
										  Организация = Неопределено, 
	...
	Если УчитываетсяКакЗапас Тогда
		...
		Если СтатьяПриростаУбылиЗапасов = СтатьяОборудование Тогда
			...
			Если (СтатьяПриростаУбылиЗапасов = СтатьяОборудование) ИЛИ НЕ ЗначениеЗаполнено(СтатьяПриростаУбылиЗапасов) Тогда
				
				СтатьяПриростаУбылиЗапасов = Справочники.СтатьиНалоговыхДеклараций.НП_К1_1ОСкакЗапасы;
				
			КонецЕсли;
			
		КонецЕсли;
		
		Если НалоговоеНазначение.Амортизируется Тогда
			
			НалоговоеНазначение = НалоговыйУчет.ПолучитьНалоговоеНазначениеПоСоставляющим(Ложь,
			                                                                НалоговоеНазначение.ВидНалоговойДеятельности,
																			НалоговоеНазначение.ВидДеятельностиНДС)	
	...
	Иначе
		 ...
		 Если (ЗначениеЗаполнено(НалоговоеНазначение)) 
			И (НЕ НалоговоеНазначение.Амортизируется) Тогда
		 	
			НалоговоеНазначение = НалоговыйУчет.ПолучитьНалоговоеНазначениеПоСоставляющим(Истина,
			                                                                НалоговоеНазначение.ВидНалоговойДеятельности,
																			НалоговоеНазначение.ВидДеятельностиНДС)	
...
КонецПроцедуры // ПриИзмененииУчитываетсяКакЗапас()
...
Функция СчетМожноИспользоватьВПроводках(Счет, Сообщать = Истина) Экспорт
	...
	Если Счет.ЗапретитьИспользоватьВПроводках Тогда
		...
		Если Сообщать Тогда
			Предупреждение(Локализация.СтрШаблон1("Счет ¤1¤ ""¤2¤"" нельзя использовать в проводках.", СокрЛП(Счет), Счет.Наименование));
		КонецЕсли;
...
Процедура УстановкаТекущейСтраницы(Объект, Панель, СтраницыСТабЧастями, СтраницаПоУмолчанию = Неопределено) Экспорт
	...
	Если ТекущаяСтраницаНеУстановлена Тогда
		Если СтраницаПоУмолчанию = Неопределено Тогда
			СтраницаПоУмолчанию = Панель.Страницы[0].Имя;
		КонецЕсли;
...
Процедура ОткрытьПодборНоменклатуры(ФормаДокумента, СтруктураПараметров, МетаданныеДокумента, ФормаПодбора = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ФормаПодбора) Тогда
...
Процедура ИзменитьЗначениеСкладаВТабЧастях(ДокОбъект, НовыйСклад, ТекЗначСклад, СтрТабЧасти, ИмяРеквСклад = "Склад") Экспорт
		...
		Для Каждого ТабЧасть Из СтруктТабЧастей Цикл
			Для Каждого СтрокаТЧ Из ДокОбъект[ТабЧасть.Ключ] Цикл
				Если (ЕстьСовпадающЗнач И СтрокаТЧ[ИмяРеквСклад] = ТекСклад)
					...
					Попытка // для РН запомним склад резерва
						Если НЕ ЗначениеЗаполнено(СтрокаТЧ.Склад1) И ЗначениеЗаполнено(НовыйСклад) Тогда
							 СтрокаТЧ.Склад1 = СтрокаТЧ.Склад;
						 КонецЕсли;
					Исключение
					КонецПопытки;
	...
	Иначе 
		Если ТипЗнч(ДокОбъект) = Тип("ДокументОбъект.РеализацияТоваровУслуг") Тогда 
			ДокОбъект.СнесколькихСкладов = Истина;
		КонецЕсли;	
	КонецЕсли;
...
//
Процедура ОткрытьФормуПроверкиШтрихкодов(Форма, ДокументОбъект, ПараметрыСвязиСтрокТЧ, ИмяТЧ = "Товары",инфа = "") Экспорт
	...
	Иначе
		...
		формаОбработки.КомментарийИзРеализации = инфа;
		Попытка  
			формаОбработки.Контрагент = ДокументОбъект.Контрагент.Наименование;
		Исключение 
		КонецПопытки;
