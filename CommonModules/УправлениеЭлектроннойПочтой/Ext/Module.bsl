...
Функция  ОтправитьПисьмо(УчЗапись,Тема,Текст,Контрагент)  Экспорт 
	...
	// Адреса Контрагента
	Адрес = ОпределитьАдресПолучателя(Контрагент);
    Сообщение = Новый ИнтернетПочтовоеСообщение; 
    Сообщение.Получатели.Добавить(Адрес); 
    Сообщение.Отправитель.Адрес = УчЗапись.АдресЭлектроннойПочты; 
	Сообщение.ИмяОтправителя = СтрЗаменить(СокрЛП(УчЗапись),УчЗапись.АдресЭлектроннойПочты,"");
    Сообщение.Тема = Тема; 
	...
	Сообщение.ОбработатьТексты();
 	...
 	Попытка
		...
		Сообщить("Письмо отправлено "+ Адрес);
		Отправили = Истина;
	Исключение
	КонецПопытки;
...
Функция ОтправитьПисьмо1(УчетнаяЗаписьОтправкиСообщений, ТекстПисьма, АдресЭлектроннойПочты, Тема = "",Вложение = "") Экспорт
	Если НЕ ЗначениеЗаполнено(АдресЭлектроннойПочты) Тогда
		#Если Клиент Тогда
		Сообщить("Не заполнен e-mail, письмо не отправлено "+АдресЭлектроннойПочты);
		# КонецЕсли
		Возврат Ложь;
	КонецЕсли;
	Отправили = Ложь;
	ПочтовыйПрофиль = ПолучитьИнтернетПочтовыйПрофиль(УчетнаяЗаписьОтправкиСообщений); 
	...
	Исключение 
		#Если Клиент Тогда
		Сообщить("Не удалось подключиться, письмо не отправлено "+АдресЭлектроннойПочты);
		# КонецЕсли
		Возврат Ложь; 
	КонецПопытки; 
	...
	ПочтовоеСообщение.Отправитель.Адрес = УчетнаяЗаписьОтправкиСообщений.АдресЭлектроннойПочты; 
	ТекстСообщения = ПочтовоеСообщение.Тексты.Добавить(ТекстПисьма);
	ТекстСообщения.ТипТекста = ТипТекстаПочтовогоСообщения.ПростойТекст;
	ПочтовоеСообщение.Получатели.Добавить(АдресЭлектроннойПочты);
	Если ЗначениеЗаполнено(Вложение) Тогда
		ПочтовоеСообщение.Вложения.Добавить(Вложение,ИмяФайлаБезПути(Вложение));
	КонецЕсли;
	Попытка
	...
	Исключение
		#Если Клиент Тогда
		Сообщить("Не удалось отправить письмо "+ СокрЛП(АдресЭлектроннойПочты));
		# КонецЕсли
		Возврат ЛОжь; 
	КонецПопытки;
	Почта.Отключиться();
	Сообщить("Письмо отправлено "+СокрЛП(АдресЭлектроннойПочты));
	Возврат Отправили;
КонецФункции
			...
			Если Получить Тогда
				...
				Если МассивЗаголовковПисем.Количество() > 0 Тогда
					...
					Для каждого ИнтернетПисьмо Из МассивПисем Цикл
						...
						КонецПопытки;
						Попытка
							Если ЗначениеЗаполнено(Письмо.ЗадачаНа) Тогда
								ОбщегоНазначения.СоздатьЗадачуНаПользователя(Письмо.Ссылка,
																			"Новое письмо",Письмо.Тема,Письмо.ЗадачаНа,Истина);
							КонецЕсли;
						Исключение
...
Процедура ПрименитьДействиеФильтра(Письмо, ПодходящийФильтр, ТекущийПользователь)
	...
	Для каждого СтрокаТЧ Из ПодходящийФильтр.ДействияФильтра Цикл
		...
		ИначеЕсли СтрокаТЧ.ДействиеФильтра = Перечисления.ВидыДействийФильтровЭлектронныхПисем.УстановитьПисьмуФлагРассмотрено Тогда
			Письмо.НеРассмотрено = Ложь;
			Письмо.ОТветственный = ТекущийПользователь;
		//ИначеЕсли СтрокаТЧ.ДействиеФильтра = Перечисления.ВидыДействийФильтровЭлектронныхПисем.у Тогда
		//	Письмо.Оформление = СтрокаТЧ.Оформление;
		ИначеЕсли СтрокаТЧ.ДействиеФильтра = Перечисления.ВидыДействийФильтровЭлектронныхПисем.СоздатьЗадачу Тогда
			Письмо.ЗадачаНа = СтрокаТЧ.Пользователь;
		КонецЕсли;
...
Функция ПреобразоватьТекстИзХТМЛФорматаВПростой(ТекстВФорматеХТМЛ) Экспорт
	...
	Попытка
		Текст = СтрЗаменить(НовыйHTMLДокумент.all.item(0).innerText, Символ(13), "");
	Исключение
		Текст = "";
	КонецПопытки;
...
Функция ПолучитьИнтернетПочтовыйПрофиль(УчетнаяЗапись) Экспорт
	...
	Если УчетнаяЗапись.POPSSL ТОгда
		Профиль.ИспользоватьSSLPOP3=Истина;
		Профиль.ИспользоватьSSLIMAP=Истина;
	КонецЕсли;
	
	Если УчетнаяЗапись.SMTPSSL ТОгда
