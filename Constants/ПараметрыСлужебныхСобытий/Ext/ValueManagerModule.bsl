...
Процедура Обновить(ЕстьИзменения = Неопределено, ТолькоПроверка = Ложь) Экспорт
	...
	Попытка
		...
		Параметры = ОбновлениеИнформационнойБазыБитрикс.ПараметрыРаботыПрограммы(
		//
		...
		Если Параметры.Свойство("ОбработчикиСобытий") Тогда
			...
			Если НЕ ОбщегоНазначенияКлиентСервер.ДанныеСовпадают(ОбработчикиСобытий, Сохраненные) Тогда
				//
				Сохраненные = Неопределено;
			КонецЕсли;
		КонецЕсли;
		...
		Если Сохраненные = Неопределено Тогда
			...
			ОбновлениеИнформационнойБазыБитрикс.УстановитьПараметрРаботыПрограммы(
			//
