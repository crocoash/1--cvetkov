					...
					Пока ВыборкаСделка.Следующий() Цикл  //Сделка
						...
						Иначе
							...
							Если ВыборкаСделка.ЗаТару Тогда
								ТекстСообщения = 
										  Локализация.СтрШаблон1("Контрагент: ¤1¤, Договор: ¤2¤"
...
"Не указана схема налогового учета, либо в схеме не указан Момент определения базы НДС по продажам!", ВыборкаДоговора.Контрагент, ВыборкаДоговора.ДоговорКонтрагента); 
		...
		Если ВыдатьПредупраждающееСообщение Тогда
				ТекстСообщения = Локализация.СтрШаблон1("Контрагент: ¤1¤, Договор: ¤2¤, Дата:  ¤3¤"
...
Процедура ЗаполнитьШапкуНалоговогоДокумента(НалоговыйДокумент, ВидОперации, Данные)
	...
	Иначе		
		...
		Если глЗначениеПеременной("ДатаВступленияВСилуПриказа1379") <= ДатаДокумента Тогда
...
#Если Клиент Тогда			      
		Если ПечататьДокументы Тогда
			Если мНеЗаписыватьДокументы = Истина Тогда
				// придется записать...
				НалоговыйДокумент.Записать(РежимЗаписиДокумента.Запись);
			КонецЕсли;
