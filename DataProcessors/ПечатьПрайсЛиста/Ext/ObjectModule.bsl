...
Процедура ЗаполнитьПостроительОтчетаПоЦенамНоменклатуры() Экспорт
	...
	|										ТОГДА 0
	|									ИНАЧЕ ЦеныРасчетные.Цена * (1 + ЦеныБазовые.ПроцентСкидкиНаценки / 100 * ВЫБОР
	|											КОГДА НЕ УточнениеЦенПоНоменклатуре.УточняющийПроцент ЕСТЬ NULL
	|												ТОГДА УточнениеЦенПоНоменклатуре.УточняющийПроцент / 100
	|											КОГДА НЕ УточнениеЦенПоРодителю1.УточняющийПроцент ЕСТЬ NULL
	|												ТОГДА УточнениеЦенПоРодителю1.УточняющийПроцент / 100
	|											КОГДА НЕ УточнениеЦенПоРодителю2.УточняющийПроцент ЕСТЬ NULL
	|												ТОГДА УточнениеЦенПоРодителю2.УточняющийПроцент / 100
	|											КОГДА НЕ УточнениеЦенПоРодителю3.УточняющийПроцент ЕСТЬ NULL
	|												ТОГДА УточнениеЦенПоРодителю3.УточняющийПроцент / 100
	|											КОГДА НЕ УточнениеЦенПоРодителю4.УточняющийПроцент ЕСТЬ NULL
	|												ТОГДА УточнениеЦенПоРодителю4.УточняющийПроцент / 100
	|											КОГДА НЕ УточнениеЦенПоРодителю5.УточняющийПроцент ЕСТЬ NULL
	|												ТОГДА УточнениеЦенПоРодителю5.УточняющийПроцент / 100
	|											ИНАЧЕ 1
	|										КОНЕЦ)
	|								КОНЕЦ
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.УточнениеРасчетаЦенПоНоменклатуре.СрезПоследних(&Дата, ) КАК УточнениеЦенПоНоменклатуре
	|		ПО ЦеныБазовые.Номенклатура = УточнениеЦенПоНоменклатуре.Номенклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.УточнениеРасчетаЦенПоНоменклатуре.СрезПоследних(&Дата, ) КАК УточнениеЦенПоРодителю1
	|		ПО ЦеныБазовые.Номенклатура.Родитель = УточнениеЦенПоРодителю1.Номенклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.УточнениеРасчетаЦенПоНоменклатуре.СрезПоследних(&Дата, ) КАК УточнениеЦенПоРодителю2
	|		ПО ЦеныБазовые.Номенклатура.Родитель.Родитель = УточнениеЦенПоРодителю2.Номенклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.УточнениеРасчетаЦенПоНоменклатуре.СрезПоследних(&Дата, ) КАК УточнениеЦенПоРодителю3
	|		ПО ЦеныБазовые.Номенклатура.Родитель.Родитель.Родитель = УточнениеЦенПоРодителю3.Номенклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.УточнениеРасчетаЦенПоНоменклатуре.СрезПоследних(&Дата, ) КАК УточнениеЦенПоРодителю4
	|		ПО ЦеныБазовые.Номенклатура.Родитель.Родитель.Родитель.Родитель = УточнениеЦенПоРодителю4.Номенклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.УточнениеРасчетаЦенПоНоменклатуре.СрезПоследних(&Дата, ) КАК УточнениеЦенПоРодителю5
	|		ПО ЦеныБазовые.Номенклатура.Родитель.Родитель.Родитель.Родитель.Родитель = УточнениеЦенПоРодителю5.Номенклатура
	|";
...
Процедура ВывестиСтрокуПрайса(Выборка, НомерГруппировки, КолонкиРезультата)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТипЗнч(ЗначениеГруппировки) = Тип("СправочникСсылка.ХарактеристикиНоменклатуры") И Ненулевые Тогда
			Ост = ОбщегоНазначения.ОстатокНоменклатуры(ЗначениеГруппировки.Владелец,Новый Массив,ЗначениеГруппировки,РабочаяДата);
			Рез = ОбщегоНазначения.РезервНоменклатуры(ЗначениеГруппировки.Владелец,Новый Массив,ЗначениеГруппировки,РабочаяДата,Документы.ЗаказПокупателя.ПустаяСсылка());
			Если Ост-Рез<=0 Тогда
				//Сообщить(ЗначениеГруппировки);
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		...
		Если ЭтоНижнийУровень Тогда
			...
			Пока ВыборкаТипЦен.Следующий() Цикл
				Если ЗначениеЗаполнено(ВыборкаТипЦен.Цена) Тогда
					...
					//Скидка
					ПроцСкидки = ОбщегоНазначения.АвтомСкидка(ЗначениеГруппировки,ДатаПрайса);
					Если ПроцСкидки>0 Тогда
						Цена = ВыборкаТипЦен.Цена *(1-ПроцСкидки/100);
					Иначе
						Цена = ВыборкаТипЦен.Цена;
					КонецЕсли;

					СекцияТипЦен.Параметры.Цена = ОбщегоНазначения.ФорматСумм(Ценообразование.ОкруглитьЦену(Цена, ВыборкаТипЦен.ПорядокОкругления, ВыборкаТипЦен.ОкруглятьВБольшуюСторону)) + СтрВалюта;
				Иначе
