...
Процедура ЗагрузитьКурсы(ИндикаторФормы = Неопределено , НадписьВалютыФормы = Неопределено, ВыводитьСообщения = Ложь ) Экспорт
	...
	Для каждого СтрокаСпВалют из СписокВалют Цикл
		...
		Если ВыводитьСообщения Тогда 
			Сообщить(Локализация.СтрШаблон1(НСтр("ru='Загружается курс для ¤1¤ (код ¤2¤).';uk='Завантажується курс для ¤1¤ (код ¤2¤).'"), СокрЛП(ТекВалюта.Наименование), ТекВалюта.Код)); 	
		КонецЕсли;
		...
		Если ЗапроситьФайлыССервера(СерверИсточник, СтрокаПараметраПолучения, ИмяВходящегоФайла, HTTP) <> Истина Тогда
			Сообщить(Локализация.СтрШаблон1(НСтр("ru='Не удалось получить ресурс для валюты ¤1¤ (код ¤2¤). Курс для валюты не загружен.';uk='Не вдалося одержати ресурс для валюти ¤1¤ (код ¤2¤). Курс для валюти не завантажений.'"), СокрЛП(ТекВалюта.Наименование), ТекВалюта.Код)); 
		...
		Если ВыводитьСообщения Тогда 
			Сообщить(Локализация.СтрШаблон1(НСтр("ru='Курс для ¤1¤ (код ¤2¤) загружен.';uk='Курс для ¤1¤ (код ¤2¤) завантажений.'"), СокрЛП(ТекВалюта.Наименование), ТекВалюта.Код)); 	
		КонецЕсли;
...
Процедура ЗагрузитьКурсыСИТС(ИндикаторФормы = Неопределено ,НадписьВалютыФормы = Неопределено, ВыводитьСообщения = Ложь ) Экспорт
	Если БукваДиска = "" Тогда 
		Сообщить(Локализация.СтрШаблон1(НСтр("ru='Не указана буква привода дисков CD-ROM.';uk='Не зазначена буква приводу дисків CD-ROM.'"))); 
	ИначеЕсли НачДата < НачалоГода(ДобавитьМесяц(ТекущаяДата(), -12)) ИЛИ КонДата > КонецМесяца(ДобавитьМесяц(ТекущаяДата(), -1)) Тогда
		Сообщить(Локализация.СтрШаблон1(НСтр("ru='На диске ИТС доступны курсы с ¤1¤ по ¤2¤. Курс для валюты не загружен.';uk='На диску ІТС доступні курси з ¤1¤ по ¤2¤. Курс для валюти не завантажений.'"), Формат(НачалоГода(ДобавитьМесяц(ТекущаяДата(), -12)),"ДЛФ=Д") , Формат(КонецМесяца(ДобавитьМесяц(ТекущаяДата(), -1)),"ДЛФ=Д"))); 	
	Иначе
