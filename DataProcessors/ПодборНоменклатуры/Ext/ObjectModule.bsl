...
Процедура ВременнаяТаблицаОстаткиНоменклатуры(Запрос, РегистрыОстатков)
	...
	|	СУММА(ЕСТЬNULL(ТоварыНаСкладах.КоличествоОстаток, 0))       КАК ФактическийОстаток,
...
Функция ЗапросОстаткиНоменклатуры(Запрос, Организация)
	...
	|	Подбор.ФактическийОстаток                       КАК ФактическийОстаток,
	|	Подбор.КоличествоСвободныйОстаток               КАК КоличествоСвободныйОстаток,
	|	Подбор.КоличествоОстатокОрганизации             КАК КоличествоОстатокОрганизации,
	...
	|	СправочникНоменклатура.Ссылка                            КАК Номенклатура,
	|	СправочникНоменклатура.Родитель                          КАК Родитель,
	|	СУММА(Остатки.ФактическийОстаток)                        КАК ФактическийОстаток,
	...
	//|	ИЛИ
	//|	(Остатки.ФактическийОстаток > 0"+?(УправлениеДопПравамиПользователей.РазрешеноПревышениеОстаткаТоваровОрганизации(),""," И Остатки.КоличествоОстатокОрганизации > 0")+"))
	|		СправочникНоменклатура.Родитель = &Родитель
	|		И (СправочникНоменклатура.ЭтоГруппа
	|				ИЛИ Остатки.ФактическийОстаток > 0)
	|
	|СГРУППИРОВАТЬ ПО
...
Процедура мОбновитьНадписьФомулаСумма(Форма) Экспорт
	...
	Иначе
		ИтоговаяСумма = 0;
		ИтоговоеКолВоВКрое1 = 0;
		ИтоговоеКолВоВКрое2 = 0;
		...
		Для Каждого СтрокаТабличнойЧасти Из Форма.ТаблицаХарактеристикНоменклатуры Цикл
			...
			Если Характеристика <> "<без характеристики>" Тогда		
				...
				Если ЗначениеЗаполнено(Ростовка) И Ростовка.ИНКоличество = 0 Тогда 
					Если СтрокаТабличнойЧасти.РазмерЕстьВКрое1 Тогда	
						ИтоговоеКолВоВКрое1 = ИтоговоеКолВоВКрое1 + СтрокаТабличнойЧасти.Количество;
					КонецЕсли;
				
					Если СтрокаТабличнойЧасти.РазмерЕстьВКрое2 Тогда
						ИтоговоеКолВоВКрое2 = ИтоговоеКолВоВКрое2 + СтрокаТабличнойЧасти.Количество;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		...
		Форма.ЭлементыФормы.ИтоговоеКолВоВКрое1.Заголовок = ИтоговоеКолВоВКрое1;
		Форма.ЭлементыФормы.ИтоговоеКолВоВКрое2.Заголовок = ИтоговоеКолВоВКрое2;
	...
	Если Форма.ЭлементыФормы.Найти("ТаблицаХарактеристикНоменклатуры") <> Неопределено Тогда
		...
		Для Каждого СтрокаТаблицыХарактеристик Из Форма.ТаблицаХарактеристикНоменклатуры Цикл
			...
			Если СтрокаТаблицыХарактеристик.Количество <> 0 Тогда
				...
				Если СтруктураИсходныхПараметров.Свойство("ДатаДокумента") И
					 СтруктураИсходныхПараметров.Свойство("ВременныеТаблицы") И
					 ТипЗнч(СтруктураИсходныхПараметров.ВременныеТаблицы) = Тип("Структура") И
					 СтруктураИсходныхПараметров.ВременныеТаблицы.Свойство("ДокументРеализацияТоваровИУслуг") Тогда
					
					нЦена = Ценообразование.ПолучитьЦенуНоменклатуры(
																	 Характеристика.Владелец,
																	 Характеристика,
																	 СтруктураИсходныхПараметров.ТипЦен,
																	 СтруктураИсходныхПараметров.ДатаДокумента,
																	 СтрокаТаблицыХарактеристик.ЕдиницаИзмерения,
																	 СтруктураИсходныхПараметров.ВалютаДокумента,
																	 ,
																	 ,
																	 ,
																	 СтруктураИсходныхПараметров.ДоговорКонтрагента);
					
				Иначе
					
					нЦена = СтрокаТаблицыХарактеристик.Цена;
				...
				СтруктураПараметров.Вставить("Цена"            , нЦена);
				...
				СтруктураПараметров.Вставить("ДатаИсполнения"  , СтрокаТаблицыХарактеристик.ДатаИсполнения);
				СтруктураПараметров.Вставить("ДатаИсполненияКонец", СтрокаТаблицыХарактеристик.ДатаИсполненияКонец);
		...
		Если СтруктураИсходныхПараметров.Свойство("СобиратьНК") И
			 СтруктураИсходныхПараметров.СобиратьНК = Истина Тогда
			
			////соберем НК из комплектующих
			//УжеБылНК = Новый СписокЗначений;
			//ТабНК = Новый ТаблицаЗначений;
			//ТабНК.Колонки.Добавить("НК");
			//ТабНК.Колонки.Добавить("Количество");
			//ТабНК.Колонки.Добавить("Характеристика");

			//Для Каждого Стр Из МассивСтруктурПараметров Цикл
			//	
			//	СтрНК = ОбщегоНазначения.НК_Характеристики(Стр.Характеристика);
			//	Если Не ТипЗнч(СтрНК) = Тип("Структура") Тогда Продолжить; КонецЕсли;
			//	НК = СтрНК.НК;
			//	НКХар = СтрНК.Характеристика;
			//	
			//	НайденНК = УжеБылНК.НайтиПоЗначению(НКХар);
			//	Если НЕ НайденНК = Неопределено Тогда Продолжить; КонецЕсли;
			//	УжеБылНК.Добавить(НКХар);
			//	
			//	КомплектующиеНК = ОбщегоНазначения.Комплектующие_НК_Комплектующей(Стр.Характеристика);
			//	НайденоКомплектующих = 0;
			//	МинимальноеКоличество = 100000;
			//	Для Каждого Стр1 Из МассивСтруктурПараметров Цикл   // пройде еще раз структуру для определения НК
			//		НайденаХарактеристика = КомплектующиеНК.Найти(Стр1.Характеристика,"ХарактеристикаКомплектующей");
			//		Если НЕ  НайденаХарактеристика = Неопределено Тогда
			//			Если Стр1.Количество<МинимальноеКоличество Тогда //минимальное
			//				МинимальноеКоличество = Стр1.Количество;
			//			КонецЕсли;
			//			НайденоКомплектующих = НайденоКомплектующих+1;
			//		КонецЕсли;
			//	КонецЦикла;
			//	
			//	Если НайденоКомплектующих = КомплектующиеНК.Количество() И НЕ НайденоКомплектующих=0  И НЕ МинимальноеКоличество = 100000 Тогда
			//		НС = ТабНК.Добавить();
			//		НС.НК = НК;
			//		НС.Количество = МинимальноеКоличество;
			//		НС.Характеристика = НКХар;
			//	КонецЕсли;
			//	
			//КонецЦикла;
			//
			//// обрабатываем структуру параметров
			//НужоУдалить = Новый ТаблицаЗначений;  Инд = 0;
			//НужоУдалить.Колонки.Добавить("Индекс");
			//Для Каждого Стр Из МассивСтруктурПараметров Цикл
			//	//ТипЦ = СтруктураИсходныхПараметров.ТипЦен;
			//	ЦенаОпт = Справочники.ТипыЦенНоменклатуры.НайтиПоКоду("000000001");
			//	//ЦенаМОпт = Справочники.ТипыЦенНоменклатуры.НайтиПоКоду("000000014");
			//	//Если ТипЦ = ЦенаОпт Тогда // если опт то мелкоопт не для НК
			//	//	Цена =  Ценообразование.ПолучитьЦенуНоменклатуры(Стр.Характеристика.Владелец,Стр.Характеристика,ЦенаМОпт,ТекущаяДата());
			//	//	ПроцСкидки = ОбщегоНазначения.АвтомСкидка(Стр.Характеристика);
			//	//	Если ПроцСкидки>0 Тогда
			//	//		Цена = Цена *(1-ПроцСкидки/100);
			//	//	КонецЕсли;
			...
			//	СтрНК = ОбщегоНазначения.НК_Характеристики(Стр.Характеристика);
			//	
			//	Если Не ТипЗнч(СтрНК) = Тип("Структура") Тогда Продолжить; КонецЕсли; // не НК
			//	НК = СтрНК.НК;
			//	НКХар = СтрНК.Характеристика;
			//	
			//	КомплектующиеНК = ОбщегоНазначения.Комплектующие_НК_Комплектующей(Стр.Характеристика);
			//	
			//	НайденНК = ТабНК.Найти(НКХар,"Характеристика");
			//	Если НЕ НайденНК = Неопределено Тогда
			//		Если Стр.Количество>НайденНК.Количество Тогда
			//			Стр.Количество = Стр.Количество-НайденНК.Количество;
			//		Иначе
			//			НС = НужоУдалить.Добавить();
			//			НС.Индекс = Инд; 
			//		КонецЕсли;
			//	КонецЕсли;
			//	Инд = Инд+1;
			//КонецЦикла;
			//
			//НужоУдалить.Сортировать("Индекс Убыв");
			//Для Каждого Стр Из НужоУдалить Цикл
			//	МассивСтруктурПараметров.Удалить(Стр.Индекс); 
			//КонецЦикла;
			//
			//// теперь добавим сами НК
			//ДЛя Каждого Стр Из ТабНК Цикл
			//	//Если ТипЦ = Справочники.ТипыЦенНоменклатуры.НайтиПоКоду("000000001") Тогда //оптовые
			//		// цена нк = сумма цен компл по цене мелкоопт 000000014
			//		Цена = 0;
			//		КомплектующиеНК = ОбщегоНазначения.Комплектующие_НК(Стр.Характеристика);
			//		Для Каждого Ком Из КомплектующиеНК Цикл
			//			Цен =  Ценообразование.ПолучитьЦенуНоменклатуры(Ком.Комплектующая,Ком.ХарактеристикаКомплектующей,ЦенаОпт,ТекущаяДата());
			//			ПроцСкидки = ОбщегоНазначения.АвтомСкидка(Ком.ХарактеристикаКомплектующей);
			//			Если ПроцСкидки>0 Тогда
			//				Цен = Цен *(1-ПроцСкидки/100);
			//			КонецЕсли;
			//			Цена = Цена+Цен;// суммируем
			//		КонецЦикла;
			//	//Иначе  // такая как в документе для НК
			//	//	Цена =  Ценообразование.ПолучитьЦенуНоменклатуры(Стр.Характеристика.Владелец,Стр.Характеристика,ТипЦ,ТекущаяДата());
			//	//	ПроцСкидки = ОбщегоНазначения.АвтомСкидка(Стр.Характеристика);
			//	//	Если ПроцСкидки>0 Тогда
			//	//		Цена = Цена *(1-ПроцСкидки/100);
			//	//	КонецЕсли;
			...
			//				
			//	СтруктураПараметров = Новый Структура();
			//	СтруктураПараметров.Вставить("ЕдиницаИзмерения", Стр.НК.ЕдиницаДляОтчетов);
			//	СтруктураПараметров.Вставить("Количество",       Стр.Количество);
			//	СтруктураПараметров.Вставить("Цена",             Цена);
			//	СтруктураПараметров.Вставить("ВалютаЦены",       Справочники.Валюты.НайтиПоКоду("980"));
			...
			пДатаИсполнения = Дата("00010101");
			пДатаИсполненияКонец = Дата("00010101");
			...
			Для Каждого СтруктураПараметров Из МассивСтруктурПараметров Цикл
				...
				//пДатаИсполнения = Макс(пДатаИсполнения, СтруктураПараметров.ДатаИсполнения);
				//пДатаИсполненияКонец = Макс(пДатаИсполненияКонец, СтруктураПараметров.ДатаИсполненияКонец);
			...
			МассивСтруктурПараметроНов = Новый Массив;
			
			ТипКомплект     = Перечисления.ТипыНоменклатуры.Комплект;
			//ТипЦенМелкийОпт = Справочники.ТипыЦенНоменклатуры.НайтиПоКоду("000000014");
			
			пСерия      = Справочники.СерииНоменклатуры.ПустаяСсылка();
			пКачество   = Справочники.Качество.Новый;
			//пВалютаЦены = СтруктураИсходныхПараметров.ВалютаДокумента;
			пВалютаЦены = ?(СтруктураИсходныхПараметров.Свойство("ВалютаДокумента"), СтруктураИсходныхПараметров.ВалютаДокумента, Форма.ВалютаЦены);
			
			Для Каждого СтрокаТЗ Из ТаблицаКомплектов Цикл
				
				пЦена = 0;
				
				//Если СтруктураИсходныхПараметров.ТипЦен.Код = "000000001" И
				//	 СтрокаТЗ.Номенклатура.ВидНоменклатуры.ТипНоменклатуры <> ТипКомплект Тогда
				Если ЗначениеЗаполнено(СтруктураИсходныхПараметров.ТипЦен) И
					 СтрокаТЗ.Номенклатура.ВидНоменклатуры.ТипНоменклатуры <> ТипКомплект Тогда
					
					пЦена = Ценообразование.ПолучитьЦенуНоменклатуры(
				...
				СтруктураПараметров.Вставить("Характеристика"  , СтрокаТЗ.Характеристика);
				СтруктураПараметров.Вставить("Серия"           , пСерия);
				СтруктураПараметров.Вставить("Качество"        , пКачество);
				СтруктураПараметров.Вставить("ЕдиницаИзмерения", СтрокаТЗ.Номенклатура.ЕдиницаХраненияОстатков);
				СтруктураПараметров.Вставить("Количество"      , СтрокаТЗ.Количество);
				СтруктураПараметров.Вставить("Цена"            , пЦена);
				СтруктураПараметров.Вставить("ВалютаЦены"      , пВалютаЦены);
				
				ДатаИсполнения =  Дата(1, 1, 1);
				ДатаИсполненияКонец =  Дата(1, 1, 1);
				
			
				ДатуИзготовления = СтрокаТЗ.Характеристика.ПолучитьОбъект().ПолучитьДатуИзготовления(Истина);
				
				Если ЗначениеЗаполнено(ДатуИзготовления) Тогда 
					СтруктураПараметров.Вставить("ДатаИсполнения"  , ДатуИзготовления.ДатаПошиваНачало);
					СтруктураПараметров.Вставить("ДатаИсполненияКонец"  ,  ДатуИзготовления.ДатаПошиваКонец);
				КонецЕсли;
				МассивСтруктурПараметроНов.Добавить(СтруктураПараметров);
				
			КонецЦикла;
			Форма.Закрыть(МассивСтруктурПараметроНов);
...
Процедура мКоманднаяПанельТаблицыХарактеристикНоменклатурыЗаполнить(Форма) Экспорт
	...
	ПодборПоСправочнику            = (Форма.ВладелецФормы.ПодборПоСправочнику Или Форма.ВладелецФормы.СписокВидовПодбора = "ОстаткиНоменклатуры");
	...
	Если СтруктураИсходныхПараметров.Свойство("ЭтоЗаказ") Тогда
		
		Форма.ТаблицаХарактеристикНоменклатуры.ЗаполнитьЗначения(Истина, "ДоступенКПродаже");
		
		Если СтруктураИсходныхПараметров.Свойство("ПроверитьКонкуренцию") И
			 ЗначениеЗаполнено(СтруктураИсходныхПараметров.Контрагент) И
			 СтруктураИсходныхПараметров.Контрагент.ТорговыеТочки.Количество() > 0 Тогда
			
			ЭтоКомплект = (Форма.Номенклатура.ВидНоменклатуры.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Комплект);
			
			ЦеноваяГруппа = Форма.Номенклатура.ЦеноваяГруппа;
			
			Если ЭтоКомплект Тогда
				
				Выборка = РегистрыСведений.КомплектующиеНоменклатуры.Выбрать(Новый Структура("Номенклатура", Форма.Номенклатура));
			...
			Если ЗначениеЗаполнено(ЦеноваяГруппа) Тогда
				...
				Для Каждого СтрокаТЧ Из СтруктураИсходныхПараметров.Контрагент.ТорговыеТочки Цикл
					...
					Запрос.Текст = 
						...
						|	&Регион В ИЕРАРХИИ
						|		(ВЫБРАТЬ РАЗЛИЧНЫЕ
						|			КонтрольПродажиЦеновыхГруппПоРегионам.ОтслеживатьКонкуренцию
						|		ИЗ
						|			РегистрСведений.КонтрольПродажиЦеновыхГруппПоРегионам КАК КонтрольПродажиЦеновыхГруппПоРегионам
						|		ГДЕ
						|			КонтрольПродажиЦеновыхГруппПоРегионам.ЦеноваяГруппа = &ЦеноваяГруппа
						|			И КонтрольПродажиЦеновыхГруппПоРегионам.ОтслеживатьКонкуренцию <> ЗНАЧЕНИЕ(Справочник.Регионы.ПустаяСсылка)) КАК РегионыСвязаны";
					...
					Если Выборка.Следующий() Тогда
						...
						Если Выборка.РегионыСвязаны Тогда
							
							Для Каждого СтрокаТЗ Из Форма.ТаблицаХарактеристикНоменклатуры Цикл
								
								Если Не ЗначениеЗаполнено(СтрокаТЗ.Характеристика) Или
									 СтрокаТЗ.ДоступенКПродаже = Ложь Тогда
									Продолжить;
								КонецЕсли;
								
								НеДоступенКПродаже = Ложь;
								...
								Если ЭтоКомплект Тогда
									...
									БИАС.ПолучитьКомплектующиеНК(СтрокаТЗ.Характеристика, ТаблицаКомплектующих);
								...
								Иначе
									...
									Характеристики.Добавить(СтрокаТЗ.Характеристика);
								...
								Пока ВыборкаЗапроса.Следующий() Цикл
									...
									Для Каждого ТорговаяТочка Из ВыборкаЗапроса.Контрагент.ТорговыеТочки Цикл
										...
										Запрос.Текст = 
											...
											|			ГДЕ
											|				КонтрольПродажиЦеновыхГруппПоРегионам.ЦеноваяГруппа = &ЦеноваяГруппа
											|				И КонтрольПродажиЦеновыхГруппПоРегионам.ОтслеживатьКонкуренцию <> ЗНАЧЕНИЕ(Справочник.Регионы.ПустаяСсылка))
											|		И (&Регион1 В ИЕРАРХИИ (&Регион2)
											|			ИЛИ &Регион2 В ИЕРАРХИИ (&Регион1)) КАК РегионыСвязаны";
										
										
										Запрос.УстановитьПараметр("Регион1"      , СтрокаТЧ.Регион);
										Запрос.УстановитьПараметр("Регион2"      , ТорговаяТочка.Регион);
										Запрос.УстановитьПараметр("ЦеноваяГруппа", ЦеноваяГруппа);
									...
									Если НеДоступенКПродаже Тогда
										
										СтрокаТЗ.ДоступенКПродаже = Ложь;
