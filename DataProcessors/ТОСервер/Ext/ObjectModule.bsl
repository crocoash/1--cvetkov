...
//  <ПеречислениеСсылка.ТООшибки*> - Результат работы функции.
//
Функция ОбработатьВнешнееСобытие(Событие, Данные, Клиент, Источник="Сканер штрихкода") Экспорт
...
Функция ОбработатьВведенныйШтрихкод(Штрихкод, Клиент) Экспорт
	...
	НайденаГруппа = Справочники.ГруппыХарактеристик.НайтиПоРеквизиту("Штрихкод", Штрихкод);
	
	Если ЗначениеЗаполнено(НайденаГруппа) Тогда
		Клиент.СШКГруппаХарактеристики(НайденаГруппа, 1);
		Возврат Истина;
	КонецЕсли;
	...
	|ГДЕ
	|    (ВЫРАЗИТЬ(СпрСН.Код КАК СТРОКА(" + Формат(СтрДлина(ШК), "ЧГ=0") + "))) = &Код");
...
Функция ОбработатьСобытиеСШК(Обработка, Объект, Клиент, Событие, Данные, СШК)
	...
	//Сообщить("исходний Данные " + Данные + " "+ СШК);
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Если ТипЗнч(РезультатЗапроса.Владелец) = Тип("СправочникСсылка.ИнформационныеКарты") Тогда
			...
			Если Ответ Тогда
				Возврат Результат;
			КонецЕсли; 
		ИначеЕсли ТипЗнч(РезультатЗапроса.Владелец) =  Тип("СправочникСсылка.ГруппыХарактеристик") Тогда     
			
		     ГруппаХарактеристик = РезультатЗапроса.Владелец.ПолучитьОбъект(); 
			  
			 Нк = ГруппаХарактеристик.НКГруппы(); 
			 
			 Хар = Справочники.ГруппыХарактеристик.НКХарактетистика(ГруппаХарактеристик); 
			 
			 Если Хар = Неопределено Тогда 
				 Предупреждение("не найдена характеристика для " + ГруппаХарактеристик);
			 КонецЕсли;
			 
			 //Сообщить("отладка ШК " + ШК + " - ГруппаХарактеристик " + ГруппаХарактеристик + "Нк " + Нк + "Хар " + Хар);
			 
			 
			 Ответ = Клиент.СШКНоменклатура(Нк,
			 Хар,
			 РезультатЗапроса.СерияНоменклатуры,
			 РезультатЗапроса.Качество,
			 РезультатЗапроса.ЕдиницаИзмерения,
			 1,
			 //СШК);
			 ?(Строка(Клиент.ЭтотОбъект) = "ОбработкаОбъект.ПечатьЭтикеток", ШК, СШК));
			 Если Ответ Тогда
				 Возврат Результат;
			 КонецЕсли;
			 
		 ИначеЕсли ТипЗнч(РезультатЗапроса.Владелец) = Тип("СправочникСсылка.Номенклатура")

			 И РезультатЗапроса.Владелец.ВидНоменклатуры.Код = "000000010" Тогда 
			 МассивКомлектующих = Справочники.Номенклатура.ПолучитьКомплектующиеПоШтрихкоду(ШК);
			 Если МассивКомлектующих <> Неопределено Тогда
				 
				 Для каждого Штр Из МассивКомлектующих Цикл
					 
					 Если Штр = ШК Тогда
						 Продолжить;
					 КонецЕсли;
					 
					 Рез = ОбработатьСобытиеСШК(Обработка, Объект, Клиент, Событие, Штр, СШК);
					 								 
				 КонецЦикла; 
				 
				 Если Рез = мНетОшибки Тогда
					 Возврат Результат;
				 КонецЕсли;
		 ...
		 Иначе  //Если ТипЗнч(РезультатЗапроса.Владелец) = ТипЗнч(РезультатЗапроса.Владелец)
			 Ответ = Клиент.СШКНоменклатура(РезультатЗапроса.Владелец,
			 РезультатЗапроса.ХарактеристикаНоменклатуры,
			 РезультатЗапроса.СерияНоменклатуры,
			 РезультатЗапроса.Качество,
